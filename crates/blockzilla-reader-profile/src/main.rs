...
use blockzilla_indexer_v3_read_sdk::IndexerV3Archive;
use blockzilla_query_sdk::{BlockSink, BlockView, ScanRange, ScanReceipt, ScanRequest};
        ...
        match self {
            Self::Count { blocks, tx, inner } => {
                let counts = block.counts.ok_or_else(|| {
                    blockzilla_query_sdk::Error::InvalidStream("native count view missing".into())
                })?;
                *blocks += 1;
