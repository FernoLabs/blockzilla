...
struct LegacyArchiveV2CompactMetaV1 {
    ...
    inner_instructions: Option<Vec<blockzilla_compact::CompactInnerInstructions>>,
    logs: Option<blockzilla_compact::CompactLogStream>,
    pre_token_balances: Vec<blockzilla_compact::CompactTokenBalance>,
    post_token_balances: Vec<blockzilla_compact::CompactTokenBalance>,
    ...
    return_data: Option<blockzilla_compact::CompactReturnData>,
    compute_units_consumed: Option<u64>,
    cost_units: Option<u64>,
}

impl LegacyArchiveV2CompactMetaV1 {
    fn into_current(self) -> anyhow::Result<blockzilla_compact::CompactMetaV1> {
        Ok(blockzilla_compact::CompactMetaV1 {
            err: self
                .err
                .as_deref()
                .map(blockzilla_compact::CompactTransactionError::from_stored_wincode_bytes)
                .transpose()?,
            fee: self.fee,
...
pub fn canonicalize_archive_v2_metadata_owned(
    ...
    let (value, schema) = match (current.ok(), legacy.ok()) {
        ...
        (Some(current), Some(legacy)) => {
            let current_bytes =
                wincode::config::serialize(&current, blockzilla_primitives::wincode_leb128_config())?;
            let legacy_bytes = wincode::config::serialize(&legacy, blockzilla_primitives::wincode_leb128_config())?;
            anyhow::ensure!(
    ...
    let output = wincode::config::serialize(&value, blockzilla_primitives::wincode_leb128_config())?;
    anyhow::ensure!(
...
/// owned [`blockzilla_compact::CompactTransactionError`], so selective readers can apply
...
fn decode_admitted_metadata_error_prefix(
...
) -> ArchiveV2WireRewriteResult<blockzilla_compact::CompactTransactionError> {
    ...
    let error = match selected.schema {
        ArchiveV2WireMetadataErrorSchema::Current => {
            <blockzilla_compact::CompactTransactionError as SchemaRead<ArchiveV2WireBoundedConfig>>::get(
                &mut prefix,
            )
            .map_err(|error| {
        ...
        ArchiveV2WireMetadataErrorSchema::Legacy => {
            ...
            blockzilla_compact::CompactTransactionError::from_stored_wincode_bytes(stored).map_err(|error| {
...
impl<V: ArchiveV2WireRewriteVisitor> Transformer<'_, V> {
    ...
    fn message_body<const PRE_UNKNOWN_FALLBACKS: bool, const TRANSCODE_PRE_TO_POST: bool>(
        ...
        if v0 {
            let lookup_count = self.sequence_len::<blockzilla_compact::OwnedCompactAddressTableLookup>(
    ...
    fn metadata_tail(&mut self) -> ArchiveV2WireRewriteResult<()> {
        ...
        if self.option_tag("metadata inner-instructions option")? {
            let group_count = self.sequence_len::<blockzilla_compact::CompactInnerInstructions>(
                "metadata inner-instruction group count",
            )?;
            for _ in 0..group_count {
                self.scalar::<u32>("metadata inner-instruction group index")?;
                let instruction_count = self.sequence_len::<blockzilla_compact::CompactInnerInstruction>(
    ...
    fn token_balances(&mut self, count_context: &'static str) -> ArchiveV2WireRewriteResult<()> {
        let count = self.sequence_len::<blockzilla_compact::CompactTokenBalance>(count_context)?;
        for _ in 0..count {
    ...
    fn rewards(&mut self) -> ArchiveV2WireRewriteResult<()> {
        let count = self.sequence_len::<blockzilla_compact::CompactReward>("metadata reward count")?;
        for _ in 0..count {
            self.pubkey(ArchiveV2WireReferenceClass::Eligible)?;
    ...
    fn logs(&mut self) -> ArchiveV2WireRewriteResult<()> {
        ...
        let event_count = self.sequence_len::<blockzilla_compact::LogEvent>("metadata log-event count")?;
        ...
        let array_count = self.sequence_len::<blockzilla_compact::DataArray>("metadata log data-array count")?;
    ...
    fn program_log(&mut self) -> ArchiveV2WireRewriteResult<()> {
        ...
        match tag {
            0 => {}
            1 => self.copy_owned::<blockzilla_program_logs::token::TokenLog>("token program log")?,
            2 => self.token_2022_log()?,
            3 => self.copy_owned::<blockzilla_program_logs::associated_token_account::TokenLog>(
                "associated-token program log",
            )?,
            4 => self
                .copy_owned::<blockzilla_program_logs::address_lookup_table::AddressLookupTableLog>(
                    "address-lookup-table program log",
                )?,
            5 => self.copy_owned::<blockzilla_program_logs::loader_v3::LoaderV3Log>(
                "loader-v3 program log",
            )?,
            6 => self.copy_owned::<blockzilla_program_logs::loader_v4::LoaderV4Log>(
                "loader-v4 program log",
            )?,
            7 => self.copy_owned::<blockzilla_program_logs::memo::MemoLog>("memo program log")?,
            8 => self.copy_owned::<blockzilla_program_logs::record::RecordLog>("record program log")?,
            9 => self.copy_owned::<blockzilla_program_logs::transfer_hook::TransferHookLog>(
                "transfer-hook program log",
            )?,
            10 => self
                .copy_owned::<blockzilla_program_logs::account_compression::AccountCompressionLog>(
                    "account-compression program log",
                )?,
            11 => {
                self.copy_owned::<blockzilla_program_logs::stake::StakeProgramLog>("stake program log")?
            }
            12 => self.copy_owned::<blockzilla_program_logs::zk_elgamal_proof::ZkElgamalProofLog>(
            ...
            17 => {
                ...
                self.copy_owned::<blockzilla_program_logs::known_programs::KnownProgramLog>(
                    "known-program log",
                )?;
    ...
    fn token_2022_log(&mut self) -> ArchiveV2WireRewriteResult<()> {
        match self.enum_tag("token-2022 log variant")? {
            0 => self.copy_owned::<blockzilla_program_logs::token_2022::Token2022ErrorLog>(
                "token-2022 error log",
            )?,
            1 => self.copy_owned::<blockzilla_program_logs::token_2022::Token2022StaticLog>(
    ...
    fn system_program_log(&mut self) -> ArchiveV2WireRewriteResult<()> {
        match self.enum_tag("system-program log variant")? {
            0 => self.copy_owned::<blockzilla_program_logs::system_program::SystemInstructionLog>(
...
mod tests {
    use super::*;
    use blockzilla_program_logs::{ProgramLog, system_program, token_2022};
    use crate::{ArchiveV2ComputeBudgetInstructionData, ArchiveV2HotInstruction, ArchiveV2HotInstructionData, ArchiveV2HotLegacyMessage, ArchiveV2HotMessagePayload, ArchiveV2HotV0Message, ArchiveV2SystemInstructionData, ArchiveV2VoteHashRef, ArchiveV2VoteLockoutOffset, ArchiveV2VoteStateUpdate, ArchiveV2VoteTowerSync};
use blockzilla_compact::{CompactInnerInstruction, CompactInnerInstructions, CompactLogStream, CompactMessageHeader, CompactMetaV1, CompactReturnData, CompactReward, CompactTokenBalance, DataArray, DataTable, LogEvent, OwnedCompactAddressTableLookup, OwnedCompactRecentBlockhash};
use blockzilla_primitives::{StringTable, wincode_leb128_config};
    ...
    fn all_key_bearing_log_events(offset: u32) -> Vec<LogEvent> {
        ...
        events.push(LogEvent::ProgramLog(ProgramLog::Known(
            blockzilla_program_logs::known_programs::KnownProgramLog::Drift(
                blockzilla_program_logs::known_programs::drift::DriftLog::Event(vec![1, 2, 3]),
            ),
        )));
        events
    }
    ...
    fn current_error_metadata_streams_to_exact_owned_bytes() {
        ...
        source_value.err = Some(blockzilla_compact::CompactTransactionError::AccountInUse);
        ...
        expected_value.err = Some(blockzilla_compact::CompactTransactionError::AccountInUse);
        let expected =
    ...
    fn unambiguous_legacy_error_metadata_streams_to_current_owned_bytes() {
        ...
        let mut expected_value = metadata_fixture(DELTA);
        expected_value.err = Some(blockzilla_compact::CompactTransactionError::InstructionError(
            7,
    ...
    fn selected_error_prefix_splitter_borrows_current_and_legacy_tails() {
        ...
        current_value.err = Some(blockzilla_compact::CompactTransactionError::InstructionError(
            7,
    ...
    fn selected_current_schema_preserves_its_prefix_and_identity_bytes() {
        let mut source_value = metadata_fixture(0);
        source_value.err = Some(blockzilla_compact::CompactTransactionError::InstructionError(
            7,
            blockzilla_compact::CompactInstructionError::BorshIoError("current-error".to_owned()),
        ));
        let source = wincode::config::serialize(&source_value, wincode_leb128_config()).unwrap();
        let mut expected_value = metadata_fixture(DELTA);
        expected_value.err = Some(blockzilla_compact::CompactTransactionError::InstructionError(
    ...
    fn selected_error_string_obeys_the_runtime_item_limit_without_partial_state() {
        ...
        value.err = Some(blockzilla_compact::CompactTransactionError::InstructionError(
            0,
            blockzilla_compact::CompactInstructionError::BorshIoError("0123456789abcdef".to_owned()),
        ));
    ...
    fn admitted_current_metadata_does_not_probe_the_legacy_error_prefix() {
        ...
        let fixture = |offset| CompactMetaV1 {
            err: Some(blockzilla_compact::CompactTransactionError::InsufficientFundsForFee),
            fee: 0,
    ...
    fn malformed_selected_error_tail_requests_owned_rollback_without_partial_state() {
        let mut value = metadata_fixture(0);
        value.err = Some(blockzilla_compact::CompactTransactionError::AccountInUse);
    ...
    fn fallback_invalid_and_trailing_failures_are_distinct_and_transactional() {
        ...
        metadata.err = Some(blockzilla_compact::CompactTransactionError::AccountInUse);
        ...
        metadata.logs = Some(CompactLogStream {
            events: vec![
                ...
                LogEvent::ProgramLog(ProgramLog::Known(
                    blockzilla_program_logs::known_programs::KnownProgramLog::Drift(
                        blockzilla_program_logs::known_programs::drift::DriftLog::Event(vec![
                            0x6b;
                            32 << 10
                        ]),
                    ),
