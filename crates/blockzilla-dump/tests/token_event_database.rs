...
fn database_implements_the_query_sdk_block_sink() {
    ...
    blockzilla_query_sdk::BlockSink::visit_block(&mut database, empty_block(0).as_view()).unwrap();
...
fn block_sink_recovers_after_a_tracker_error() {
    ...
    assert!(
        blockzilla_query_sdk::BlockSink::visit_block(&mut database, invalid.as_view()).is_err()
    );
    blockzilla_query_sdk::BlockSink::visit_block(&mut database, valid.as_view()).unwrap();
    assert_eq!(database.next_block_ordinal(), 1);
}
...
fn resume_rejects_a_wrong_source_or_opening_generation() {
    ...
    let wrong_opening = TargetMintTrackerSnapshot::from_parts(
        ...
        [(
            SOURCE_ACCOUNT,
            blockzilla_query_sdk::token::TargetAccountSnapshot {
