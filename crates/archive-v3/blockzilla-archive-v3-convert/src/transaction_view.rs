...
impl TransactionArenaEncoder {
    ...
    pub fn prepare(&mut self, transactions: &[Transaction]) -> Result<PreparedTransactionArena> {
        ...
        for (index, transaction) in transactions.iter().enumerate() {
            ...
            if let Err(error) = blockzilla_archive_v3::ledger::transactions::append_transaction(
                &mut rows,
                transaction,
            ) {
                rows.clear();
...
mod tests {
    ...
    fn empty_effects(transaction_count: usize) -> [EffectFileIndex; EFFECT_KIND_COUNT] {
        let chunks = transaction_count.div_ceil(
            blockzilla_archive_v3::ledger::transactions::EFFECT_CHUNK_TRANSACTIONS as usize,
        );
    ...
    fn unavailable_states(transaction_count: usize) -> Vec<EffectState> {
        vec![
            EffectState::new(blockzilla_archive_v3::ledger::transactions::CpiState::Unavailable,);
            transaction_count
        ]
    }
    ...
    fn raw_arena_and_encoder_scratch_recover_after_error_and_recycle() {
        ...
        assert_eq!(
            blockzilla_archive_v3::ledger::transactions::decode_transactions(
