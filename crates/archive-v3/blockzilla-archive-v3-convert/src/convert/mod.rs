...
use blockzilla_archive_v2::{
    ...
    read_archive_v2_hot_block_index_file,
};
...
/// Command options for converting one Compact V2 generation into Archive V3.
///
/// Both the Blockzilla command and the compatibility binary use these options.
/// [`run`] checks resource limits and fixture evidence before it opens the source.
#[derive(Debug, Clone, clap::Args)]
pub struct ConvertOptions {
    /// Published Compact V2 generation directory, or an explicit local fixture.
    #[arg(value_name = "V2_GENERATION_DIR")]
    pub source: PathBuf,
    /// New candidate directory. This path and its staging path must not exist.
    #[arg(value_name = "OUTPUT_DIR")]
    pub output: PathBuf,
    /// Worker limit for the conversion pipeline.
    #[arg(long, default_value_t = default_workers())]
    pub workers: usize,
    /// Pipeline and index sort memory limit in MiB, excluding some dictionaries.
    #[arg(long, default_value_t = 4_096)]
    pub pipeline_memory_limit_mib: usize,
    /// Expected source epoch. Required for an unpublished fixture.
    #[arg(long)]
    pub epoch: Option<u64>,
    /// Expected slots per epoch. Required for an unpublished fixture.
    #[arg(long)]
    pub slots_per_epoch: Option<u64>,
    /// Admit a local test input without a published generation manifest.
    #[arg(long)]
    pub fixture_source: bool,
    /// Fixture message grammar: current or may24-pre-unknown-fallbacks.
    #[arg(long, value_parser = parse_fixture_message_schema)]
    pub fixture_message_schema: Option<CompactV2MessageSchema>,
    /// Fixture metadata grammar: current-typed-error or legacy-raw-error.
    #[arg(long, value_parser = parse_fixture_metadata_schema)]
    pub fixture_metadata_schema: Option<CompactV2MetadataSchema>,
    /// Fixture predecessor hash as 64 hex characters. Requires a predecessor slot.
    #[arg(long = "fixture-previous-blockhash-hex", value_parser = parse_hex_32)]
    pub fixture_previous_blockhash: Option<[u8; 32]>,
    /// Fixture predecessor slot. Requires a predecessor hash.
    #[arg(long)]
    pub fixture_previous_slot: Option<u64>,
    /// Convert only this many blocks for a non-publishable benchmark candidate.
    #[arg(long)]
    pub benchmark_prefix_blocks: Option<usize>,
}
...
impl Plane {
    fn create(root: &Path, name: &'static str) -> Result<Self> {
        Ok(Self {
            ...
            bytes: blockzilla_archive_v3::FILE_HEADER_LEN as u64,
            pages: 0,
            compressed_pages: 0,
            raw_pages: 0,
        })
...
fn derive_content_archive_id(root: &Path, epoch: u64) -> Result<ArchiveId> {
        ...
        .filter(|spec| spec.class == FileClass::Canonical)
    {
        ...
        if spec.encoding == FileEncoding::HeaderedBinary {
            ...
            let header = blockzilla_archive_v3::FileHeader::decode(&header_bytes)
...
fn patch_archive_id(root: &Path, old: ArchiveId, new: ArchiveId) -> Result<()> {
        ...
        .filter(|spec| spec.encoding == FileEncoding::HeaderedBinary)
    {
        ...
        file.read_exact_at(&mut bytes, 0)
            .with_context(|| format!("read {} header", path.display()))?;
        let mut header = blockzilla_archive_v3::FileHeader::decode(&bytes)
            .with_context(|| format!("decode {} header", path.display()))?;
        header
...
/// Convert one generation and print its conversion report as JSON.
///
/// The source admission, staging, and complete candidate checks are the same
/// for both command entry points. The output remains an unpublished candidate.
pub fn run(options: ConvertOptions) -> Result<()> {
    let conversion_started = Instant::now();
    let args = options.validate()?;
    ...
    let mut report = Report {
        ...
        metadata_source_profile: match source_context.metadata_schema {
            CompactV2MetadataSchema::CurrentTypedError => "archive-v2-current-typed-error-v1",
            CompactV2MetadataSchema::LegacyRawError => "archive-v2-legacy-raw-error-v1",
        },
        poh_source_schema: match retained_poh.source_schema {
            crate::source_v2_sidecars::SourcePohSchema::Current => {
                "archive-v2-current-wincode-0.5.5"
            }
            crate::source_v2_sidecars::SourcePohSchema::LegacyNoSignatureCount => {
                "archive-v2-legacy-no-signature-count-wincode-0.5.5"
            }
            crate::source_v2_sidecars::SourcePohSchema::NoEntrySchemaEvidence => {
    ...
    if args.benchmark_prefix_blocks.is_some() {
        ...
        report.plane_bytes.insert(
            blockzilla_archive_v3::sidecars::poh::PATH.to_owned(),
            retained_poh.object.file_bytes,
        );
        report.poh_entries = retained_poh.entry_count;
        report.plane_bytes.insert(
            blockzilla_archive_v3::sidecars::shredding::PATH.to_owned(),
    ...
    let signatures = copy_file_payload(
        ...
        &signatures_src,
        &output,
        blockzilla_archive_v3::sidecars::signatures::PATH,
        archive_id,
        signature_records,
    )
    .context("copy signatures")?;
    report.plane_bytes.insert(
        blockzilla_archive_v3::sidecars::signatures::PATH.to_owned(),
    ...
    report.poh_signature_count_legacy_unknown_blocks = retained_poh
        ...
        .filter(|mapping| {
            mapping.signature_count_coverage
                == crate::source_v2_sidecars::BlockSignatureCountCoverage::LegacyUnknown
        })
        .count() as u64;
    report.plane_bytes.insert(
        blockzilla_archive_v3::sidecars::shredding::PATH.to_owned(),
...
mod tests {
    ...
    use blockzilla_program_logs::{
        program_logs::ProgramLog, program_logs::system_program::NonceAction,
        program_logs::system_program::PubkeyOrString, program_logs::system_program::SystemAddress,
        program_logs::system_program::SystemInstructionLog,
    ...
    fn options_with(arguments: &[&str]) -> ConvertOptions {
        TestCli::try_parse_from(
            ["convert", "source", "output", "--workers", "1"]
                .into_iter()
    ...
    #[test]
    fn convert_options_keep_the_existing_defaults() {
        let options = TestCli::try_parse_from(["convert", "source", "output"])
            .unwrap()
            .options;
        assert_eq!(options.source, Path::new("source"));
        assert_eq!(options.output, Path::new("output"));
        assert_eq!(options.workers, default_workers());
        assert_eq!(options.pipeline_memory_limit_mib, 4_096);
        assert!(!options.fixture_source);
        assert!(options.epoch.is_none());
        assert!(options.benchmark_prefix_blocks.is_none());
    }

    #[test]
    fn convert_options_parse_explicit_fixture_evidence() {
        let args = options_with(&[
        ...
        .validate()
        .unwrap();
        assert_eq!(args.pipeline_memory_limit_bytes, 32 << 20);
        assert_eq!(args.epoch, Some(7));
        assert_eq!(args.slots_per_epoch, Some(432_000));
        assert_eq!(args.fixture_previous_blockhash, Some([0xab; 32]));
        assert_eq!(args.fixture_previous_slot, Some(3_023_999));
        assert_eq!(args.benchmark_prefix_blocks, Some(16));
        assert_eq!(
            args.fixture_message_schema,
            Some(CompactV2MessageSchema::May24PreUnknownFallbacks)
        );
        assert_eq!(
            args.fixture_metadata_schema,
            Some(CompactV2MetadataSchema::LegacyRawError)
        );
    }

    #[test]
    fn convert_options_reject_invalid_resource_limits_before_io() {
        let base = options_with(&[]);
        for workers in [0, MAX_ORDERED_PARALLEL_DECODE_WORKERS + 1] {
            let mut options = base.clone();
            options.workers = workers;
            assert!(run(options).unwrap_err().to_string().contains("--workers"));
        }
        for mib in [0, usize::MAX] {
            let mut options = base.clone();
            options.pipeline_memory_limit_mib = mib;
            assert!(
                run(options)
                    .unwrap_err()
                    .to_string()
                    .contains("--pipeline-memory-limit-mib")
            );
        }
        let mut options = base;
        options.benchmark_prefix_blocks = Some(0);
        assert!(
            run(options)
                .unwrap_err()
                .to_string()
                .contains("--benchmark-prefix-blocks")
        );
    }

    #[test]
    fn convert_options_keep_fixture_evidence_restrictions() {
        let base = options_with(&[]);
        for (hash, slot) in [(Some([1; 32]), None), (None, Some(1))] {
            let mut options = base.clone();
            options.fixture_source = true;
            options.fixture_previous_blockhash = hash;
            options.fixture_previous_slot = slot;
            assert!(
                options
                    .validate()
                    .unwrap_err()
        ...
        predecessor.fixture_previous_blockhash = Some([1; 32]);
        predecessor.fixture_previous_slot = Some(1);
        let mut message = base.clone();
        message.fixture_message_schema = Some(CompactV2MessageSchema::Current);
        let mut metadata = base;
        metadata.fixture_metadata_schema = Some(CompactV2MetadataSchema::CurrentTypedError);
        for options in [predecessor, message, metadata] {
            assert!(
                options
                    .validate()
                    .unwrap_err()
                    .to_string()
                    .contains("only with --fixture-source")
            );
        }
    }

    #[test]
    fn convert_options_reject_unknown_grammars_and_invalid_hashes() {
        for (option, value) in [
            ("--fixture-message-schema", "auto"),
            ("--fixture-metadata-schema", "auto"),
            ("--fixture-previous-blockhash-hex", "abc"),
        ] {
            assert!(
                TestCli::try_parse_from(["convert", "source", "output", option, value]).is_err()
            );
        }
    }
    ...
    fn b3_source_projection_rejects_loaded_and_block_reward_ids() {
        ...
        let payload =
            ArchiveV2HotMessagePayload::V0(blockzilla_archive_v2::ArchiveV2HotV0Message {
                header: blockzilla_compact::CompactMessageHeader {
                    num_required_signatures: 1,
                    num_readonly_signed_accounts: 0,
                    num_readonly_unsigned_accounts: 0,
