...
The converter supports the Compact V2 current-hot source profile:
...
The converter stops when it finds one of these source forms:
...
```text
cargo run --release -p blockzilla -- archive-v3 convert \
  <compact-v2-generation> <candidate-directory> \
  --workers <N> --pipeline-memory-limit-mib <MiB>
```

The `blockzilla-archive-v3-convert` binary remains available as a compatibility
command. It accepts the same arguments and uses the same conversion engine.
...
An unpublished test source has no manifest that can select its message and
metadata wire schemas. It must name both frozen schemas explicitly:

```text
cargo run --release -p blockzilla -- archive-v3 convert \
  <compact-v2-generation> <candidate-directory> \
  --fixture-source --epoch <N> --slots-per-epoch <N> \
  --fixture-message-schema current \
  --fixture-metadata-schema current-typed-error
```

Use `may24-pre-unknown-fallbacks` for a fixture with the historical message
schema, or `legacy-raw-error` for a fixture with the historical metadata schema.
The converter does not probe messages to choose a schema. Raw instructions
...
```text
cargo run --release -p blockzilla -- \
  archive-v3 build-basic-indexes <candidate-directory>

cargo run --release -p blockzilla -- \
  archive-v3 account-index <candidate-directory>

cargo run --release -p blockzilla -- \
  archive-v3 program-index <candidate-directory>

cargo run --release -p blockzilla -- \
  archive-v3 selector-index <candidate-directory>
```
...
```text
cargo run --release -p blockzilla-read-archive-v3 \
