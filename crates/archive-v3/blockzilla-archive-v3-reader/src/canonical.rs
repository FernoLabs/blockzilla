//! Local readers for the canonical Archive V3 transaction stream and runtime effects.
//!
//! This format uses `catalog/` and `ledger/`. It is distinct from the frozen
//! standalone prototype opened by [`crate::IndexerV3Archive`].
...
use crate::container::{decode_zstd_exact, validate_open_file};
use blockzilla_archive_v3::ledger::accounts::ResolvedAccounts;
...
mod tests {
    ...
    #[test]
    fn page_and_effect_spans_reject_stored_bytes_above_the_guard() {
        let root = tempfile::tempdir().unwrap();
        let archive_id = ArchiveId::new([5; 16]);
        let spec = blockzilla_archive_v3::object_by_path(outcomes::PATH).unwrap();
        let header = FileHeader::new(spec, archive_id, 1, 2, 2).unwrap();
        let path = root.path().join(outcomes::PATH);
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        let mut bytes = header.encode().to_vec();
        bytes.extend_from_slice(&[0, 0]);
        std::fs::write(path, bytes).unwrap();
        let object = InputObject::open(root.path(), outcomes::PATH, archive_id).unwrap();
        let page_error = object
            .read_page(
                PageSpan {
                    offset: FILE_HEADER_LEN as u64,
                    stored_len: 2,
            ...
            .unwrap_err();
        assert!(page_error.to_string().contains("stored bytes"));
        let effect_error = object
            .read_effect_chunk(
                FILE_HEADER_LEN as u64,
                transactions::ChunkFrame::raw(2).unwrap(),
                1,
                "corrupt effect span",
            )
            .unwrap_err();
        assert!(effect_error.to_string().contains("stored bytes"));
    }
}
