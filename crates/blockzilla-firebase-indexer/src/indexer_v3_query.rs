...
impl ParallelScanJobResources {
    fn checked_add(self, other: Self) -> blockzilla_query_sdk::Result<Self> {
        Ok(Self {
    ...
    fn checked_sub(self, other: Self) -> blockzilla_query_sdk::Result<Self> {
        Ok(Self {
...
) -> blockzilla_query_sdk::Result<Option<ParallelScanJobResources>> {
...
struct PendingParallelResult {
    resources: ParallelScanJobResources,
    result: blockzilla_query_sdk::Result<ParallelScanJobOutput>,
}
...
impl ParallelOwnedPayloadTracker {
    fn add(&self, bytes: u64) -> blockzilla_query_sdk::Result<()> {
        let current = self
            .current
...
impl ParallelOwnedPayloadGuard {
    ...
    fn add_block(&mut self, bytes: u64) -> blockzilla_query_sdk::Result<()> {
...
) -> blockzilla_query_sdk::Result<u64> {
...
fn parallel_checked_add_payload(total: &mut u64, value: u64) -> blockzilla_query_sdk::Result<()> {
...
fn parallel_capacity_bytes<T>(capacity: usize) -> blockzilla_query_sdk::Result<u64> {
    let bytes = capacity
...
impl IndexerV3InstructionSource {
    ...
    ) -> blockzilla_query_sdk::Result<IndexerV3SelectiveScanReceipt> {
    ...
    pub fn scan_ordered_parallel_with_registry_policy(
        ...
        workers: NonZeroUsize,
        sink: &mut dyn BlockSink,
    ) -> blockzilla_query_sdk::Result<IndexerV3ParallelScanReceipt> {
        validate_parallel_worker_count(workers)?;
    ...
    fn decode_parallel_job(
        ...
        worker: usize,
    ) -> blockzilla_query_sdk::Result<ParallelScanJobOutput> {
        #[cfg(test)]
    ...
    #[cfg(test)]
    fn apply_parallel_test_hook(&self, job_id: usize) -> blockzilla_query_sdk::Result<()> {
    ...
    fn run_parallel_jobs<F>(
        ...
        mut consume: F,
    ) -> blockzilla_query_sdk::Result<ParallelScanTotals>
    where
        F: FnMut(&ParallelScanJobOutput) -> blockzilla_query_sdk::Result<()>,
    {
        validate_parallel_worker_count(workers)?;
        ...
        thread::scope(|scope| {
            ...
            loop {
                ...
                while let Some(pending_result) = pending.remove(&next_result) {
                    ...
                    let process_result = (|| -> blockzilla_query_sdk::Result<()> {
    ...
    fn project_transaction_inner(
                ...
                if request.include_execution_status {
                    let status =
                        CompactV2MetadataProjector::new(metadata_schema, context.registry_entries)
                            .project_split_outcome(
                                required_plane(transaction.outcome, "outcome")?,
                    ...
                    ProjectedMetadata::ExactStatus(status)
                } else {
                    if transaction.outcome.is_some() {
                        return Err(IndexerV3InstructionSourceError::Invalid(
                            "unrequested V3 outcome plane is present".into(),
                        ));
                    }
        ...
        // Preserve the failed header, but do not resolve keys or emit balance rows
        // when the consumer explicitly excludes failed transaction details.
...
fn validate_parallel_worker_count(workers: NonZeroUsize) -> blockzilla_query_sdk::Result<()> {
...
impl ParallelScanJobPlan {
    ...
    fn next_job(&mut self) -> blockzilla_query_sdk::Result<Option<ParallelScanJob>> {
        match self {
            Self::Ordered {
                reader,
                selection,
...
impl ParallelScanJob {
    fn descriptor(
        ...
        reader: &Reader,
        worker: usize,
    ) -> blockzilla_query_sdk::Result<ParallelAssignedDescriptor> {
        ...
        let mut record = |ordinal: usize| -> blockzilla_query_sdk::Result<()> {
...
fn parallel_block_resources(
    ...
    selection: SemanticPlaneSelection,
    include_signatures: bool,
) -> blockzilla_query_sdk::Result<ParallelScanJobResources> {
...
) -> blockzilla_query_sdk::Result<IndexerV3RegistryReadReceipt> {
...
fn validated_requested_bounds(
    identity: &SourceIdentity,
    request: &ScanRequest,
) -> blockzilla_query_sdk::Result<(u32, u32)> {
...
fn validate_selected_blocks(
    ...
    requested_end: u32,
    archive_blocks: u32,
) -> blockzilla_query_sdk::Result<Vec<usize>> {
...
mod tests {
    ...
    impl BlockSink for NullBlockSink {
        fn visit_block(
            &mut self,
            _block: blockzilla_query_sdk::BlockView<'_>,
        ) -> blockzilla_query_sdk::Result<()> {
            Ok(())
        }
    }
    ...
    #[test]
    fn status_only_token_scan_does_not_require_instruction_planes() {
        let message = || {
            legacy_message(
        ...
        let mut success = metadata(2, None, None, vec![], vec![]);
        success.post_token_balances.push(CompactTokenBalance {
            account_index: 0,
            mint: Some(CompactPubkey::Id(3)),
            owner: None,
            program_id: None,
        ...
        failed.post_token_balances = success.post_token_balances.clone();
        let fixture = Fixture::build(
            &[SIGNER, PROGRAM, TARGET_MINT],
            vec![vec![
                FixtureTransaction::exact(message(), success, 0),
                FixtureTransaction::exact(message(), failed, ARCHIVE_V2_TX_FLAG_HAS_ERROR),
                FixtureTransaction::absent(message()),
                FixtureTransaction::raw_metadata(message()),
            ]],
            None,
            false,
            100,
        ...
        for omit_failed in [false, true] {
            ...
            let planes = semantic_plane_selection(&request);
            assert!(planes.outcomes && planes.token_balances);
            assert!(!planes.loaded_addresses && !planes.inner_instructions);
            for workers in [1, 3] {
                let mut source = fixture.open("status-only-token-scan");
                let mut blocks = Vec::new();
                source
                    .scan_ordered_parallel_with_registry_policy(
                        &request,
                ...
                let txs = &blocks[0].1;
                assert_eq!(txs.len(), 4);
                assert_eq!(txs[0].header.status, ExecutionStatus::Succeeded);
                assert_eq!(txs[0].token_balances.len(), 1);
                assert_eq!(txs[1].header.status, ExecutionStatus::Failed);
                assert_eq!(txs[1].header.failed_outer_instruction_index, Some(0));
                assert_eq!(txs[1].token_balances.len(), usize::from(!omit_failed));
                assert_eq!(
                    txs[2].header.status,
                    ExecutionStatus::Unknown(CoverageReason::MetadataAbsent)
                );
                ...
                assert!(txs.iter().all(|tx| tx.instructions.is_empty()));
            }
        }
    }
