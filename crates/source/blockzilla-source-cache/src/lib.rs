...
mod tests {
    ...
    use super::*;
    use blockzilla_source_http::HttpRangeSourceOptions;
    ...
    impl TestServer {
        ...
        fn interrupt_gets(&self, object: &str) {
            self.state
                .lock()
                .unwrap()
                .objects
    ...
    fn serve_connection(mut stream: TcpStream, state: &Arc<Mutex<ServerState>>) {
        ...
        let (served, interrupt) = {
            ...
            let interrupt = served.interrupt_gets && method == "GET";
    ...
    fn interrupted_download_removes_only_its_private_partial() {
        ...
        // Keep truncating responses so the HTTP retry policy is exhausted.
        server.interrupt_gets("sidecar.bin");
        let directory = cache_tempdir();
