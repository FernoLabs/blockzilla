...
use blockzilla_compact::{
    ...
    CompactReward, CompactShredding, CompactTransactionConfig, CompactTransactionError,
...
pub struct WincodeArchiveV2PohRecord {
    pub block_id: u32,
    pub slot: u64,
    pub entries: Vec<blockzilla_compact::CompactPohEntry>,
}
...
pub struct WincodeArchiveV2PohRecordLegacyNoSignatureCount {
    pub block_id: u32,
    pub slot: u64,
    pub entries: Vec<blockzilla_compact::CompactPohEntryLegacyNoSignatureCount>,
}
...
pub struct WincodeArchiveV2ShreddingRecord {
    pub block_id: u32,
    pub slot: u64,
    pub shredding: Vec<blockzilla_compact::CompactShredding>,
}
...
mod hot_block_slot_time_tests {
    ...
    fn poh_record_decoder_supports_current_encoding() {
        let record = WincodeArchiveV2PohRecord {
            block_id: 42,
            slot: 999,
            entries: vec![blockzilla_compact::CompactPohEntry {
                num_hashes: 10,
                hash: [7; 32],
                tx_count: 2,
                signature_count: 3,
    ...
    fn poh_record_decoder_falls_back_to_pre_signature_count_encoding() {
        let record = WincodeArchiveV2PohRecordLegacyNoSignatureCount {
            block_id: 42,
            slot: 999,
            entries: vec![blockzilla_compact::CompactPohEntryLegacyNoSignatureCount {
                num_hashes: 10,
                hash: [7; 32],
                tx_count: 2,
            }],
    ...
    fn bounded_current_decoder_respects_reward_allocation_limit() {
        ...
        assert_eq!(
            borrowed.message_bytes.as_ptr(),
            current.message_bytes.as_ptr()
        );
        assert_eq!(
            borrowed.metadata_bytes.as_ptr(),
            current.metadata_bytes.as_ptr()
        );
    }
