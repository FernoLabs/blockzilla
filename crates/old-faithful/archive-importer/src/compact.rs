...
use of_car_reader::car_stream::CarStream;
...
use of_car_reader::{car_block_group::CarBlockGroup, error::GroupError};

use blockzilla_format::{
    BlockhashRegistry, CompactAddressTableLookup, CompactBlockHeader, CompactDataFrame,
    CompactInstruction, CompactLegacyMessage, CompactMessage, CompactMessageHeader,
    CompactPohEntry, CompactRecentBlockhash, CompactShredding, CompactTransaction,
    CompactTxWithMeta, CompactV0Message, KeyIndex, KeyStore, PostcardFramedWriter, Signature,
    compact_meta_from_proto,
};

use crate::{BUFFER_SIZE, Cli, ProgressTracker, epoch_paths, genesis_epoch0};
...
pub(crate) fn run(cli: &Cli, epoch: u64) -> Result<()> {
    ...
    let blockhash_id_offset = match genesis_epoch0::maybe_load_for_epoch(&cli.cache_dir, epoch)? {
        Some(genesis)
            if hashes
                .first()
                .is_some_and(|hash| hash == &genesis.genesis_hash) =>
        {
            1
        }
        Some(_) => {
            warn!(
                "Epoch 0 genesis archive exists, but blockhash registry does not start with genesis hash; compact header ids use legacy offset 0"
            );
            0
        }
        None => 0,
...
fn compact_process_block_manual<W: std::io::Write>(
    ...
    if group.poh_hashes.len() != group.poh_num_hashes.len()
        || group.poh_hashes.len() != group.entry_tx_counts.len()
    {
        return Err(GroupError::Other(
            "entry PoH vectors and tx counts are out of sync".to_string(),
        ));
    }
    ...
    let shredding = group
        ...
        .collect();
    let poh_entries = group
        .poh_num_hashes
        .iter()
        .zip(group.poh_hashes.iter())
        .zip(group.entry_tx_counts.iter())
        .map(|((num_hashes, hash), tx_count)| CompactPohEntry {
            num_hashes: *num_hashes,
            hash: *hash,
            tx_count: *tx_count,
        })
        .collect();
    let rewards = group.rewards.as_ref().map(|frame| CompactDataFrame {
        hash: frame.hash,
        index: frame.index,
        total: frame.total,
    ...
    let blockhash_id = block_i.saturating_add(blockhash_id_offset);
    let header = CompactBlockHeader {
        slot: block_slot,
        parent_slot: group.parent_slot.unwrap_or(0),
        blockhash: blockhash_id,
        previous_blockhash: blockhash_id.saturating_sub(1),
...
pub fn to_compact_transaction<'a>(
    vtx: &'a of_car_reader::versioned_transaction::VersionedTransaction,
    index: &KeyIndex,
    bh_index: &GxHashMap<[u8; 32], i32>,
    ...
    let message = match &vtx.message {
        VersionedMessage::Legacy(m) => {
            ...
            let account_keys = m.account_keys.iter().map(|a| index.compact(a)).collect();
        ...
        VersionedMessage::V0(m) => {
            ...
            let address_table_lookups = m
                ...
                .map(|lookup| CompactAddressTableLookup {
                    account_key: index.compact(lookup.account_key),
