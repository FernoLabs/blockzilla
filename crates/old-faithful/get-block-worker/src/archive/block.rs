...
use worker::{Bucket, Context, Env, Range as R2Range};
...
pub(crate) async fn get_block(
    ...
    let slot_lookup = get_slot_lookup(&index_bucket, ctx, epoch, slot)
        .await?
...
async fn get_slot_lookup(
    ...
    get_slot_lookup_from_compact_indexes(index_bucket, ctx, epoch, slot).await
}
...
async fn get_slot_lookup_from_raw_index(
    ...
    .await?
    {
        ...
        return Ok(Some(SlotLookup {
            range: entry.range,
            previous_blockhash: nonzero_blockhash(entry.previous_blockhash),
        }));
    }
...
async fn get_slot_lookup_from_compact_indexes(
    ...
    let epoch_cid = epoch_cid(index_bucket, Some(ctx), epoch).await?;
    ...
    if !missing_indexes.is_empty() {
        warm_indexes(ctx, index_bucket.clone(), missing_indexes.clone());
        return Err(FetchError::IndexWarming {
...
async fn epoch_cid(
    ...
    if let Some(ctx) = ctx {
        let index_bucket = index_bucket.clone();
        let path = path.clone();
        let bytes = cid.as_bytes().to_vec();
        ctx.wait_until(async move {
            if let Err(err) = index_bucket.put(path.clone(), bytes).execute().await {
                worker::console_error!("failed to write R2 epoch CID {path}: {err}");
            }
        });
    }
    Ok(cid)
...
fn nonzero_blockhash(blockhash: [u8; 32]) -> Option<[u8; 32]> {
    blockhash.iter().any(|byte| *byte != 0).then_some(blockhash)
}

fn warm_indexes(ctx: &Context, index_bucket: Bucket, paths: Vec<String>) {
    ctx.wait_until(async move {
        for path in paths {
            match http::get_bytes(&path).await {
                Ok(bytes) => match index_bucket.put(path.clone(), bytes).execute().await {
                    Ok(_) => worker::console_log!("warmed R2 index {path}"),
                    Err(err) => worker::console_error!("failed to write R2 index {path}: {err}"),
                },
                Err(err) => worker::console_error!(
                    "failed to fetch Old Faithful index {path}: {}",
                    err.client_message()
                ),
            }
        }
...
async fn lookup_car_entry_range_for_cid(
    ...
    let epoch_cid_path = format!("{epoch}/epoch-{epoch}.cid");
    let epoch_cid = epoch_cid(index_bucket, None, epoch)
        .await
