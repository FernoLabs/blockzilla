...
pub(crate) struct InfoAvailability {
    pub(crate) availability: Option<AvailabilityInfo>,
    pub(crate) cluster_epoch_hint_error: Option<String>,
}

#[derive(Serialize)]
pub(crate) struct AvailabilityInfo {
    latest_complete_epoch: u64,
    latest_available_slot: u64,
...
pub(crate) async fn get(env: &Env) -> InfoAvailability {
    ...
    let cluster_epoch_hint_error = hint_result.as_ref().err().map(FetchError::client_message);
    if let Some(error) = &cluster_epoch_hint_error {
        worker::console_warn!("getEpochInfo hint unavailable: {}", error);
    }
    let cluster_epoch_hint = hint_result.ok().flatten();
    let availability = latest_availability(env, cluster_epoch_hint.as_ref())
        .await
        .ok();
...
async fn latest_availability(
    env: &Env,
    cluster_epoch_hint: Option<&ClusterEpochHint>,
) -> FetchResult<AvailabilityInfo> {
    ...
    let end = cluster_epoch_hint.as_ref().map_or(fallback_end, |hint| {
        hint.candidate_complete_epoch.max(start)
    });
    let index_bucket = env.bucket("OF_INDEXES")?;

    for epoch in (start..=end).rev() {
        if let Some(info) =
            epoch_availability(&index_bucket, epoch, start, end, cluster_epoch_hint).await?
        {
            return Ok(info);
        }
    }
    ...
    Err(FetchError::MissingSlotIndex {
        key: format!("no complete Old Faithful epoch found from {start} through {end}"),
    })
}

async fn epoch_availability(
    index_bucket: &Bucket,
    epoch: u64,
    scan_start_epoch: u64,
    scan_target_epoch: u64,
    cluster_epoch_hint: Option<&ClusterEpochHint>,
) -> FetchResult<Option<AvailabilityInfo>> {
    let v2_path = format!("slot-index/epoch-{epoch}-slot-ranges-v2.raw");
    let raw_path = format!("slot-index/epoch-{epoch}-slot-ranges.raw");
    let (index_path, index_version) = if index_bucket.head(&v2_path).await?.is_some() {
        (v2_path, "v2")
    ...
    Ok(Some(AvailabilityInfo {
        latest_complete_epoch: epoch,
        latest_available_slot: last_epoch_slot,
        first_epoch_slot,
        last_epoch_slot,
        slots_per_epoch: SLOTS_PER_EPOCH,
        car_path: format!("{epoch}/epoch-{epoch}.car"),
        index_path,
        index_version,
...
fn cluster_epoch_hint_from_rpc(
    ...
    ClusterEpochHint {
        ...
        candidate_complete_epoch: epoch_from_absolute_slot.saturating_sub(1),
        slots_per_epoch,
