...
pub fn stored_meta_to_proto(m: StoredTransactionStatusMeta) -> TransactionStatusMeta {
    ...
    let rewards = m
        .rewards
        .as_ref()
        .map(|v| v.iter().map(stored_reward_to_proto).collect())
        .unwrap_or_default();
...
#[inline]
pub fn stored_reward_to_proto(r: &stored::StoredExtendedReward) -> confirmed_block::Reward {
...
#[inline]
pub fn stored_confirmed_block_reward_to_proto(
