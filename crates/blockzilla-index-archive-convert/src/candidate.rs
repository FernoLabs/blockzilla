...
mod tests {
    ...
    use blockzilla_index_archive_format::{FileEncoding, LAYOUT};
    ...
    fn cross_archive_and_missing_objects_fail() {
        ...
        write_payload(
            root.path(),
            blockzilla_index_archive_format::indexes::slots::PATH,
            ArchiveId::new([8; 16]),
            0,
            &[],
        )
        ...
        fs::remove_file(
            root.path()
                .join(blockzilla_index_archive_format::indexes::slots::PATH),
        )
        .unwrap();
        assert!(validate_complete_candidate(root.path(), 1).is_err());
    }
