...
fn open_epoch_source(
    ...
    let zero_hash_generation_digest = allow_historical_digest_compatibility
        ...
        opened_generation_digest == expected_generation_digest
...
fn write_epoch_pass_two(
    ...
    let registry_stamp = FileStamp::read(&registry_file)?;
        ...
        registry_stamp == plan.registry_stamp,
    ...
    let signatures_stamp = FileStamp::read(&signatures_file)?;
        ...
        signatures_stamp == plan.signatures_stamp,
    ...
    fs::remove_file(&plan.locator_path)
        .with_context(|| format!("remove completed locator {}", plan.locator_path.display()))?;
    fs::remove_file(&plan.source_id_map_path).with_context(|| {
        format!(
            "remove completed source ID map {}",
            plan.source_id_map_path.display()
        )
    })?;
    Ok(read_stats)
}
...
fn prepare_output_path(input: &Path, output: &Path) -> Result<PathBuf> {
    ensure!(
...
pub fn consolidate_epoch_shards_v3(archive_root: &Path, input: &Path, output: &Path) -> Result<()> {
    ...
    let output = prepare_output_path(&input, output)?;
    ...
    let mut signature_writer = DigestWriter::create(&signature_partial)?;
    ...
    fs::remove_dir(&key_work)
        .with_context(|| format!("remove completed key work directory {}", key_work.display()))?;
    fs::remove_dir(&work)
