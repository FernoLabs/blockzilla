...
After Pass 1 and the global-registry merge, the consolidator writes a durable resume plan. During
Pass 2 it commits output lengths and counters after every epoch. If Pass 2 stops, run the same
command with `--resume`. The command validates the plan, reopens the same registry, truncates an
uncommitted epoch suffix, restores the output hashes, and continues with the next epoch. A ctime
change caused only by a hard-link or permission update does not invalidate a source between
passes; inode, size, and modification time must still match. Changes while an epoch is being read
still stop the run.
...
For a new run, the output directory must not exist. `--resume` requires the incomplete output
directory and refuses a directory that already has its final manifest. The completed directory
contains only:
