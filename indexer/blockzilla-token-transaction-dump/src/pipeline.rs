...
pub struct ConsolidateConfig {
    ...
    pub input: PathBuf,
    pub output: PathBuf,
    /// The operator asserts that only source metadata changed after extraction.
    /// Consolidation still pins and verifies the registry and signature files it reads.
    pub allow_metadata_generation_drift: bool,
    /// Continue a consolidation whose durable Pass 1 plan was completed.
    pub resume: bool,
}
...
pub fn consolidate_dump(config: ConsolidateConfig) -> Result<()> {
    consolidate_epoch_shards(
        &config.archive_root,
        &config.input,
        &config.output,
        config.allow_metadata_generation_drift,
        config.resume,
    )
}
