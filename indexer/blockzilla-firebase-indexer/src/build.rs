...
pub fn scan_into_builder_parallel(
    archive: &ArchiveReader<PinnedLocalRangeSource>,
    chunk_start: u32,
    chunk_width: u32,
    max_program_id: u32,
...
pub fn scan_into_dense_accumulator(
    archive: &ArchiveReader<PinnedLocalRangeSource>,
...
fn build_dense_index_to_staging(
    ...
    archive
        .source()
        .verify_unchanged()
        .context("archive object changed during dense build pass 1")?;
...
fn finalize_index(
    ...
    archive
        .source()
        .verify_unchanged()
        .context("archive object changed while the index was being built")?;
    manifest
        .write(out_dir)
...
/// points — `blocks_range` reads/decodes strictly within `range` and
/// `PinnedLocalRangeSource::read_range` uses positioned (`pread`-style) reads
/// on retained file handles, so concurrent calls across disjoint ranges on a
/// shared `&ArchiveReader` are safe.
fn scan_signers_range(
    archive: &ArchiveReader<PinnedLocalRangeSource>,
    range: std::ops::Range<usize>,
...
pub fn discover_signers_parallel(
    archive: &ArchiveReader<PinnedLocalRangeSource>,
    thread_count: usize,
) -> Result<DiscoverSignersStats> {
...
fn discover_signer_set_builder(
    archive: &ArchiveReader<PinnedLocalRangeSource>,
    thread_count: usize,
