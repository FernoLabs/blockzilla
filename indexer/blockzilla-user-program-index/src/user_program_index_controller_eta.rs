//! Size-based ETA estimates for the user-program index controller.
//!
...
/// second User-program index worker.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
#[derive(Debug, Error)]
pub enum EtaError {
    #[error("unsupported User-program index ETA history schema {0}")]
    UnsupportedHistorySchema(u32),
    #[error("User-program index ETA history has {0} samples, above the safety limit")]
    TooManyHistorySamples(usize),
    #[error("invalid User-program index ETA sample {index}: {reason}")]
    InvalidSample { index: usize, reason: &'static str },
    #[error("invalid User-program index ETA work: {0}")]
    InvalidWork(&'static str),
    #[error("invalid User-program index ETA queue: {0}")]
