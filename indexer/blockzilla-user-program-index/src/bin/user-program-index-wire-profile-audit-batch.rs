...
//! User-program index controller lock for its full lifetime, gives the archive scheduler
...
#[cfg(test)]
use blockzilla_user_program_index::user_program_index_wire_profile_attestation::encode_receipt_source_recovery_evidence_v3;
use blockzilla_user_program_index::{
    format::RegistryFileIdentity,
    user_program_index_wire_profile_attestation::{
...
#[derive(Debug, Parser)]
#[command(
    name = "user-program-index-wire-profile-audit-batch",
    about = "Run one immutable User-program index wire-profile audit batch"
)]
struct Args {
    ...
    #[arg(long)]
    state_root: PathBuf,
    /// Existing User-program index controller root whose exclusive lock must remain held.
    #[arg(long)]
...
fn acquire_controller_lock(root: &Path) -> Result<File> {
    ...
    let result = unsafe { libc::flock(file.as_raw_fd(), libc::LOCK_EX | libc::LOCK_NB) };
        ...
        result == 0,
        "User-program index controller is running or another batch holds its lock"
    );
    let after = fs::symlink_metadata(&path)?;
    ensure!(
        same_file(&opened, &after),
...
// New children use the current tag. Exact legacy tags still bind a recovered
// child. Additional fields or conflicting aliases must not establish ownership.
fn attempt_environment_matches(environment: &[u8], attempt_id: &str) -> bool {
    let current = format!("BLOCKZILLA_USER_PROGRAM_INDEX_AUDIT_BATCH_ATTEMPT_ID={attempt_id}");
    let legacy = format!("BLOCKZILLA_FIREWATCH_AUDIT_BATCH_ATTEMPT_ID={attempt_id}");
    ...
    fields.next().is_none() && (field == current.as_bytes() || field == legacy.as_bytes())
}
...
fn process_matches_attempt_identity(
    ...
    if !attempt_environment_matches(&environment, &attempt.attempt_id) {
        return Ok(false);
    }
    let cmdline = match fs::read(format!("/proc/{pid}/cmdline")) {
        Ok(bytes) => bytes,
...
fn spawn_attempt(
    ...
    command
        ...
        .env_clear()
        .env(
            "BLOCKZILLA_USER_PROGRAM_INDEX_AUDIT_BATCH_ATTEMPT_ID",
            &attempt_id,
        )
        .stdin(Stdio::null())
...
mod tests {
    ...
    fn complete_receipt_without_current_attestation_is_rejected() {
        let root = std::env::temp_dir().join(format!(
            "user-program-index-complete-test-{}-{}",
            std::process::id(),
            unix_now()
        ));
        create_private_directory(&root).unwrap();
    ...
    fn private_directory_permissions_are_enforced() {
        let root = std::env::temp_dir().join(format!(
            "user-program-index-permission-test-{}-{}",
            std::process::id(),
            unix_now()
        ));
        let mut builder = fs::DirBuilder::new();
    ...
    #[test]
    fn attempt_environment_accepts_exact_current_or_legacy_identity() {
        for name in [
            "BLOCKZILLA_USER_PROGRAM_INDEX_AUDIT_BATCH_ATTEMPT_ID",
            "BLOCKZILLA_FIREWATCH_AUDIT_BATCH_ATTEMPT_ID",
        ] {
            let environment = format!("{name}=correct\0");
            assert!(attempt_environment_matches(
            ...
            assert!(!attempt_environment_matches(
                format!("{environment}LD_PRELOAD=hostile\0").as_bytes(),
                "correct"
            ));
        }
        assert!(!attempt_environment_matches(b"", "correct"));
        assert!(!attempt_environment_matches(
            b"BLOCKZILLA_USER_PROGRAM_INDEX_AUDIT_BATCH_ATTEMPT_ID=correct\0BLOCKZILLA_FIREWATCH_AUDIT_BATCH_ATTEMPT_ID=correct\0",
            "correct"
        ));
    }
    ...
    fn child_command_clears_hostile_loader_environment() {
        ...
        command
            .env_clear()
            .env("BLOCKZILLA_USER_PROGRAM_INDEX_AUDIT_BATCH_ATTEMPT_ID", "a");
    ...
    fn public_and_private_auditors_yield_but_exact_owned_private_child_is_allowed() {
        let root = PathBuf::from(format!(
            "/tmp/user-program-index-guard-test-{}-{}",
            std::process::id(),
            unix_now()
        ));
        create_private_directory(&root).unwrap();
