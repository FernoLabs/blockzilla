//! Resource-aware controller for the per-epoch user-program index project.
//!
//! The archive scheduler remains the priority owner of the NAS. This controller runs at most
//! a bounded adaptive pool of User-program index child process groups, yields the pool when scheduler work
...
use blockzilla_user_program_index::user_program_index_controller_cgroup;
use blockzilla_user_program_index::user_program_index_controller_eta;
...
use blockzilla_user_program_index::user_program_index_wire_profile_attestation::{
...
const MAX_USER_PROGRAM_INDEX_WORKERS: usize = 4;
...
#[derive(Debug, Parser)]
#[command(
    name = "user-program-index-controller",
    about = "Build and verify user-program indexes while giving the archive scheduler priority"
)]
struct Args {
    /// Scheduler read-only status endpoint. Loopback HTTP only.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_SCHEDULER_STATUS_URL",
    ...
    /// Root receiving immutable per-epoch user-program index generations.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_ROOT")]
    output_root: PathBuf,

    /// Parent for private index-parity sort workspaces.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_SCRATCH_ROOT")]
    parity_scratch_root: PathBuf,

    /// Deployed index-builder executable.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_INDEXER_BIN")]
    indexer_bin: PathBuf,

    /// Deployed registry-aware parity executable.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_BIN")]
    parity_bin: PathBuf,

    /// Controller-owned durable markers and logs.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_STATE_ROOT")]
    controller_state_root: PathBuf,

    /// Atomic schema-1 JSON consumed by the monitor.
    #[arg(long, env = "BLOCKZILLA_USER_PROGRAM_INDEX_STATUS_FILE")]
    status_file: PathBuf,

    /// Decoder threads passed to each dense index build.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_THREADS",
        default_value_t = 4
    )]
    threads: usize,
    ...
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_WORKER_THREADS",
        default_value_t = 2
    )]
    additional_worker_threads: usize,

    /// Maximum number of User-program index phases that adaptive admission can run together.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_MAX_WORKERS",
        default_value_t = 1
    )]
    max_workers: usize,

    /// Memory headroom reserved for each additional User-program index phase.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_WORKER_MEMORY_RESERVE_MIB",
        default_value_t = 768
    )]
    worker_memory_reserve_mib: u64,

    /// Host memory that User-program index keeps available before it adds another worker.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_HOST_MEMORY_RESERVE_MIB",
        default_value_t = 3072
    )]
    host_memory_reserve_mib: u64,

    /// Maximum host I/O full pressure used only when User-program index adds another worker.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_IO_FULL_MAX",
        default_value_t = 10.0
    )]
    additional_io_full_max: f64,

    /// Maximum host CPU pressure used only when User-program index adds another worker.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_CPU_SOME_MAX",
    ...
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_SORT_MEMORY_MIB",
        default_value_t = 256
    )]
    ...
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_POLL_SECS",
        default_value_t = 5
    )]
    poll_secs: u64,
    ...
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_RESUME_STABLE_SECS",
        default_value_t = 60
    )]
    ...
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_CLEAN_TRANSITION_SECS",
        default_value_t = 10
    )]
    clean_transition_secs: u64,

    /// Keep this much shared archive-disk space free for scheduler work and User-program index scratch.
    #[arg(
        long,
        env = "BLOCKZILLA_USER_PROGRAM_INDEX_DISK_RESERVE_GIB",
        default_value_t = 512
    )]
...
enum PressureContext {
    /// No User-program index worker is active. Any measured I/O pressure comes from other work.
    Admission,
    /// A User-program index worker is running. Its own I/O must not make it stop itself.
    Active,
    /// A User-program index worker is stopped. It adds no new I/O, so PSI is a conservative resume gate;
    /// the rolling average can still include its earlier I/O for a short time.
    Paused,
}

// The command line uses the current name. Existing service environments still
// work; an explicitly set current variable takes precedence over its old name.
fn parse_args() -> Args {
    let mut command = Args::command();
    for (argument, current, legacy) in [
        (
            "scheduler_status_url",
            "BLOCKZILLA_USER_PROGRAM_INDEX_SCHEDULER_STATUS_URL",
            "BLOCKZILLA_FIREWATCH_SCHEDULER_STATUS_URL",
        ),
        (
            "output_root",
            "BLOCKZILLA_USER_PROGRAM_INDEX_ROOT",
            "BLOCKZILLA_FIREWATCH_INDEX_ROOT",
        ),
        (
            "parity_scratch_root",
            "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_SCRATCH_ROOT",
            "BLOCKZILLA_FIREWATCH_PARITY_SCRATCH_ROOT",
        ),
        (
            "indexer_bin",
            "BLOCKZILLA_USER_PROGRAM_INDEX_INDEXER_BIN",
            "BLOCKZILLA_FIREWATCH_INDEXER_BIN",
        ),
        (
            "parity_bin",
            "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_BIN",
            "BLOCKZILLA_FIREWATCH_PARITY_BIN",
        ),
        (
            "controller_state_root",
            "BLOCKZILLA_USER_PROGRAM_INDEX_STATE_ROOT",
            "BLOCKZILLA_FIREWATCH_STATE_ROOT",
        ),
        (
            "status_file",
            "BLOCKZILLA_USER_PROGRAM_INDEX_STATUS_FILE",
            "BLOCKZILLA_FIREWATCH_STATUS_FILE",
        ),
        (
            "threads",
            "BLOCKZILLA_USER_PROGRAM_INDEX_THREADS",
            "BLOCKZILLA_FIREWATCH_THREADS",
        ),
        (
            "additional_worker_threads",
            "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_WORKER_THREADS",
            "BLOCKZILLA_FIREWATCH_ADDITIONAL_WORKER_THREADS",
        ),
        (
            "max_workers",
            "BLOCKZILLA_USER_PROGRAM_INDEX_MAX_WORKERS",
            "BLOCKZILLA_FIREWATCH_MAX_WORKERS",
        ),
        (
            "worker_memory_reserve_mib",
            "BLOCKZILLA_USER_PROGRAM_INDEX_WORKER_MEMORY_RESERVE_MIB",
            "BLOCKZILLA_FIREWATCH_WORKER_MEMORY_RESERVE_MIB",
        ),
        (
            "host_memory_reserve_mib",
            "BLOCKZILLA_USER_PROGRAM_INDEX_HOST_MEMORY_RESERVE_MIB",
            "BLOCKZILLA_FIREWATCH_HOST_MEMORY_RESERVE_MIB",
        ),
        (
            "additional_io_full_max",
            "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_IO_FULL_MAX",
            "BLOCKZILLA_FIREWATCH_ADDITIONAL_IO_FULL_MAX",
        ),
        (
            "additional_cpu_some_max",
            "BLOCKZILLA_USER_PROGRAM_INDEX_ADDITIONAL_CPU_SOME_MAX",
            "BLOCKZILLA_FIREWATCH_ADDITIONAL_CPU_SOME_MAX",
        ),
        (
            "parity_sort_memory_mib",
            "BLOCKZILLA_USER_PROGRAM_INDEX_PARITY_SORT_MEMORY_MIB",
            "BLOCKZILLA_FIREWATCH_PARITY_SORT_MEMORY_MIB",
        ),
        (
            "poll_secs",
            "BLOCKZILLA_USER_PROGRAM_INDEX_POLL_SECS",
            "BLOCKZILLA_FIREWATCH_POLL_SECS",
        ),
        (
            "resume_stable_secs",
            "BLOCKZILLA_USER_PROGRAM_INDEX_RESUME_STABLE_SECS",
            "BLOCKZILLA_FIREWATCH_RESUME_STABLE_SECS",
        ),
        (
            "clean_transition_secs",
            "BLOCKZILLA_USER_PROGRAM_INDEX_CLEAN_TRANSITION_SECS",
            "BLOCKZILLA_FIREWATCH_CLEAN_TRANSITION_SECS",
        ),
        (
            "disk_reserve_gib",
            "BLOCKZILLA_USER_PROGRAM_INDEX_DISK_RESERVE_GIB",
            "BLOCKZILLA_FIREWATCH_DISK_RESERVE_GIB",
        ),
    ] {
...
fn main() -> Result<()> {
    ...
    let cgroup_path = match resolve_self_cgroup_v2() {
        Ok(path) => Some(path),
        Err(error) => {
            tracing::warn!(%error, "adaptive User-program index capacity is limited to one worker");
            None
        }
    };
...
fn validate_args(args: &Args) -> Result<()> {
        ...
        (1..=MAX_USER_PROGRAM_INDEX_WORKERS).contains(&args.max_workers),
        "--max-workers must be between 1 and {MAX_USER_PROGRAM_INDEX_WORKERS}"
    );
    ensure!(
...
fn validate_path_arguments(args: &Args) -> Result<()> {
    ...
    for path in paths {
        ...
        "controller state root must be the scheduler legacy firewatch-index child"
    );
    ensure!(
    ...
    for writable in [
        ...
        &args.controller_state_root,
            ...
            "writable User-program index roots must not overlap the archive root"
        );
    }
    Ok(())
}
...
fn validate_created_path_layout(args: &Args) -> Result<()> {
    ...
    for (index, left) in writable.iter().enumerate() {
        for right in &writable[index + 1..] {
                ...
                "writable User-program index roots overlap"
            );
        }
    }
    Ok(())
...
fn acquire_controller_lock(root: &Path) -> Result<File> {
    ...
    let result = unsafe { libc::flock(file.as_raw_fd(), libc::LOCK_EX | libc::LOCK_NB) };
        ...
        result == 0,
        "another User-program index controller holds the lock"
    );
    Ok(file)
}
...
fn run_loop(args: &Args, client: &Client, cgroup_path: Option<&Path>) -> Result<()> {
    ...
    if let Err(error) = load_or_initialize_eta_history(args).and_then(|history| {
        EtaEstimator::from_history(&history)
            .context("build initial User-program index ETA calibration")
    }) {
        tracing::warn!(%error, "User-program index ETA calibration is unavailable; indexing will continue");
    }
    ...
    loop {
        ...
        // for archive discovery, index validation, cleanup, or another User-program index child.
                ...
                && pool.children.len() > 1
            {
                if let Ok(snapshot) = &scheduler {
                    ...
                    let reduction_reason = if snapshot.machine.memory_available_bytes < host_floor {
                        ...
                        Some(format!(
                            "User-program index memory pressure rose above 1% ({cgroup_memory_some:.2}%)"
                        ))
                    } else {
                        None
                    };
            ...
            for epoch in active_epochs {
                ...
                match outcome {
                    Ok(Some(status)) => {
                        ...
                        match finish_child(args, finished, status.success()) {
                            ...
                            Err(error) => {
                                ...
                                runtime_reason = Some(format!(
                                    "User-program index epoch {epoch} child failed: {error:#}"
                                ));
                            }
                        }
                    ...
                    Err(error) => {
                        ...
                        runtime_reason = Some(format!(
                            "User-program index epoch {epoch} child poll failed: {error}"
                        ));
                        completion_fault_seen = true;
                        pool.resume_safe_since = None;
                    }
        ...
        let views = match build_views(
            ...
            Ok(views) => views,
            Err(error) => {
                let reason = format!("User-program index state validation failed: {error:#}");
        ...
        let admission_pressure = match state_block.as_ref().or(recovery_block.as_ref()) {
            ...
            None if candidate_set_changed => PressureState::Cancel(
                "User-program index candidate set changed; waiting for scheduler reconciliation"
                    .into(),
            ),
        ...
        let cleanup_reason = if cleanup_allowed {
            match drain_one_deferred_cleanup(args) {
                Ok(reason) => reason,
                Err(error) => {
                    let reason = format!("deferred User-program index cleanup failed: {error:#}");
                    ...
                    if !stable {
                            ...
                            .and_then(|()| spawn_phase(args, &candidate, phase, worker_threads))
                        {
                            Ok(child) => {
                                    ...
                                    "User-program index admitted the same epoch twice"
                                );
                            ...
                            Err(error) => {
                                admission_reason = Some(format!(
                                    "User-program index epoch {} could not start: {error:#}",
                                    candidate.epoch
                                ));
                            }
                        }
        ...
        if let Some((spawned_epoch, was_additional, pre_spawn_cgroup)) = spawned {
            ...
            let post_spawn_pressure = match &post_spawn_scheduler {
                ...
                Err(error) => PressureState::Cancel(format!(
                    "scheduler status unavailable after User-program index spawn: {error}"
                )),
            };
        ...
        let eta_estimator = load_or_initialize_eta_history(args)
            .and_then(|history| {
                EtaEstimator::from_history(&history)
                    .context("build User-program index ETA calibration")
            })
            .map_err(|error| {
                tracing::warn!(%error, "User-program index ETA calibration is unavailable");
                error
            })
            .ok();
...
fn pressure_state(
    ...
    // Host memory PSI is not attributable while User-program index is active. The worker can create its
    ...
    // Total host PSI cannot distinguish User-program index I/O from competing I/O. Use it as an
    // admission/resume signal only, when User-program index is not generating I/O. Scheduler runnable
...
fn additional_worker_decision(
    ...
    if pool.children.values().any(|child| child.paused) {
        return AdmissionDecision::Wait("an existing User-program index worker is paused".into());
    }
...
fn cgroup_runtime_pressure(
        ...
        || current.events.oom_kill > previous.events.oom_kill
    {
        return Some(PressureState::Cancel(
            "User-program index cgroup reached a hard memory event".into(),
        ));
    }
    None
}
...
fn apply_pool_runtime_pressure(
    ...
    match pressure {
        ...
        PressureState::Pause(reason) => {
            ...
            pool_action_result("pause User-program index pool", errors)?;
        }
        PressureState::Hold => {
            pool.clear_admission_window();
            pool.resume_safe_since = None;
        ...
        PressureState::Safe => {
            if pool.children.values().any(|child| child.paused) {
                ...
                if since.elapsed() >= Duration::from_secs(args.resume_stable_secs) {
                    ...
                    pool.resume_safe_since = None;
                    pool_action_result("resume User-program index pool", errors)?;
                }
            } else {
                pool.resume_safe_since = None;
            }
...
fn cancel_pool(args: &Args, pool: &mut PoolState, reason: &str) -> Result<()> {
    ...
    pool.clear_admission_window();
    pool.resume_safe_since = None;
    pool_action_result("cancel User-program index pool", errors)
}
...
fn fail_children_with_invalid_authority(
    ...
    for (epoch, authority_error) in invalid {
        ...
        let message = format!(
            "wire-profile authority changed while the User-program index child was active: {authority_error}"
        );
    ...
    pool.resume_safe_since = None;
    pool_action_result(
        "fail User-program index children with changed authority",
        errors,
    )?;
    Ok(Some(format!(
        "wire-profile authority changed for active User-program index epochs {}",
        failed_epochs
            .iter()
            .map(u64::to_string)
            .collect::<Vec<_>>()
...
fn shed_newest_child(args: &Args, pool: &mut PoolState, reason: &str) -> Result<u64> {
    let epoch = pool
        ...
        .map(|(epoch, _)| *epoch)
        .context("cannot shed a child from an empty User-program index pool")?;
    let outcome = {
        let child = pool
            .children
            .get_mut(&epoch)
            ...
            None => match signal_verified_child_group(child, libc::SIGKILL)? {
                OwnedChildSignal::Signaled => {
                    ...
                    wait_for_signaled_child(&mut child)
                        .context("reap newest User-program index child")?;
                    defer_child_cleanup(
                        args,
                        &child,
                        false,
    ...
    pool.clear_admission_window();
    pool.resume_safe_since = None;
    pool_action_result("shed newest User-program index child", errors)?;
    Ok(epoch)
}
...
fn probe_child_exit(child: &mut ActiveChild) -> Result<Option<ExitStatus>> {
    child
        .child
        .try_wait()
        .context("poll User-program index child")
}
...
fn signal_verified_child_group(
    ...
    if observed_start_ticks != Some(child.start_ticks) {
        ...
        bail!("User-program index child process identity changed");
    }
    if process_group_id(child.pid) != Some(child.pid) {
        ...
        bail!("User-program index child is not its process-group leader");
    }
...
fn wait_for_signaled_child(child: &mut ActiveChild) -> Result<ExitStatus> {
    ...
    loop {
        if let Some(status) = child.child.try_wait()? {
            ...
            "signaled User-program index child did not exit within 10 seconds"
        );
...
fn discover_candidates(args: &Args, status: &SchedulerStatus) -> Result<CandidateDiscovery> {
    ...
    let eligible = candidates.len();
        ...
        candidates
            ...
            .len()
            == eligible,
        "User-program index discovered more than one candidate mode for one epoch"
    );
    let blocked_epochs = blocked_wire_profiles
        ...
        "User-program index discovered more than one wire-profile audit row for one epoch"
    );
    ensure!(
        candidates
            .iter()
            .all(|candidate| !blocked_epochs.contains(&candidate.epoch)),
        "User-program index epoch is both eligible and blocked on a wire-profile audit"
    );
...
fn revalidate_before_spawn(
    ...
    let candidates = discovery.candidates;
        ...
        "User-program index candidate set changed before spawn"
    );
    ensure!(
        ...
        "selected User-program index candidate changed before spawn"
    );
    ensure!(
        matches!(
            pressure_state(
        ...
        "scheduler or resource state changed before User-program index spawn"
    );
    Ok(())
}
...
fn validate_complete_registry_contract(
    ...
    if matches!(marker.schema_version, 2 | 3) {
        ...
        // give User-program index current attempt/profile provenance.
        ensure!(
    ...
    for root_name in ["epochs", "canonical-epochs"] {
        ...
        for entry in fs::read_dir(&root)? {
            ...
            let marker: WorkflowMarker = read_bounded_json(&entry.path())?;
                ...
                "invalid User-program index workflow marker for epoch {epoch}"
            );
                ...
                .filter(|child| child.candidate.mode == root_mode)
            {
                    ...
                    "in-memory User-program index child differs from its durable marker for epoch {epoch}"
                );
                continue;
            }
            if active.get(&epoch).is_some_and(|child| {
                ...
                return Ok(Some(format!(
                    "User-program index epoch {epoch} has live workflow claims in both candidate modes"
                )));
            }
            ...
            let fresh_pidless_start = marker.state == "starting"
                ...
                "active User-program index workflow marker for epoch {epoch} has no exact attempt identity"
            );
            let allowed_executable = match marker.phase {
                ...
                "User-program index workflow marker for epoch {epoch} has an unexpected executable"
            );
            ...
            if let Some(declared) = marker.owned_temp_path.as_deref() {
                ...
                if declared.exists() {
                        ...
                        "User-program index epoch {epoch} temporary workspace differs from its marker"
                    );
                }
            }
            ...
            if let (Some(pid), Some(start_ticks)) = (marker.pid, marker.process_start_ticks) {
                ...
                {
                    return Ok(Some(format!(
                        "User-program index epoch {epoch} has a live process identity that does not match its workflow"
                    )));
                }
            } else if marker.pid.is_some() || marker.process_start_ticks.is_some() {
                return Ok(Some(format!(
                    "User-program index epoch {epoch} has an incomplete process identity"
                )));
            }
            if fresh_pidless_start && matches.is_empty() && discovered_temp.is_none() {
                return Ok(Some(format!(
                    "User-program index epoch {epoch} is inside its bounded spawn-recovery window"
                )));
            }
            for (pid, start_ticks) in &matches {
                ...
                if process_start_ticks(*pid) == Some(*start_ticks) {
                    return Ok(Some(format!(
                        "User-program index epoch {epoch} orphan worker did not stop"
                    )));
                }
            }
            ...
            {
                return Ok(Some(format!(
                    "User-program index epoch {epoch} has a live temporary-workspace owner that cannot be proven as its exact attempt process"
                )));
            }
            ...
            if let Some(path) = cleanup_path.as_deref() {
                ...
                let Some(owner_pid) = owner_pid else {
                    return Ok(Some(format!(
                        "User-program index epoch {epoch} has a temporary workspace without a bound owner"
                    )));
                };
                ...
                if owner_start_ticks.is_none() && !owner_absence_confirmed {
                    return Ok(Some(format!(
                        "User-program index epoch {epoch} has an unproven live PID for a pidless cleanup attempt"
                    )));
                }
                ...
                return Ok(Some(format!(
                    "User-program index epoch {epoch} workflow recovery deferred cleanup; waiting for stable admission"
                )));
            }
            let message = if matches.is_empty() {
            ...
            return Ok(Some(format!(
                "User-program index epoch {epoch} workflow recovery completed; waiting for stable admission"
            )));
        }
    }
    Ok(None)
...
fn workflow_root_mode(root_name: &str) -> Result<CandidateMode> {
    match root_name {
        ...
        _ => bail!("unknown User-program index workflow root"),
    }
}
...
fn process_matches_marker_pid(marker: &WorkflowMarker, pid: u32) -> Result<bool> {
    ...
    let expected_attempt = format!("BLOCKZILLA_USER_PROGRAM_INDEX_ATTEMPT_ID={attempt_id}");
    let legacy_attempt = format!("BLOCKZILLA_FIREWATCH_ATTEMPT_ID={attempt_id}");
    // Accept either exact old or current tag, but reject conflicting identities.
    let mut attempt_fields = environment.split(|byte| *byte == 0).filter(|field| {
        field.starts_with(b"BLOCKZILLA_USER_PROGRAM_INDEX_ATTEMPT_ID=")
            || field.starts_with(b"BLOCKZILLA_FIREWATCH_ATTEMPT_ID=")
    });
    let Some(attempt_field) = attempt_fields.next() else {
        return Ok(false);
    };
    if attempt_fields.next().is_some()
        || (attempt_field != expected_attempt.as_bytes()
            && attempt_field != legacy_attempt.as_bytes())
    {
        return Ok(false);
    }
...
fn validate_attempt_temp_path(args: &Args, marker: &WorkflowMarker, path: &Path) -> Result<()> {
    let attempt_id = marker
        .attempt_id
        .as_deref()
        .context("active User-program index marker has no attempt ID")?;
    let (parent, prefix) = attempt_temp_parent_and_prefix(args, marker)?;
    let name = path
        .file_name()
        .and_then(|name| name.to_str())
        .context("User-program index temporary workspace has no UTF-8 name")?;
    ensure!(
        path.parent() == Some(parent.as_path())
            && name.starts_with(&prefix)
            && name.ends_with(&format!("-{attempt_id}"))
            && owned_temp_pid(path).is_some(),
        "User-program index temporary workspace is outside its exact attempt binding"
    );
    Ok(())
}
...
fn find_attempt_temp(args: &Args, marker: &WorkflowMarker) -> Result<Option<PathBuf>> {
    ...
    match fs::read_dir(&parent) {
            ...
            for entry in entries {
                ...
                if name.starts_with(&prefix) && name.ends_with(&format!("-{attempt_id}")) {
                    ...
                    let metadata = fs::symlink_metadata(&path)?;
                        ...
                        metadata.file_type().is_dir() && !metadata.file_type().is_symlink(),
                        "User-program index attempt temporary path is not a real directory"
                    );
                    matches.push(path);
                }
            }
        ...
        matches.len() <= 1,
        "multiple User-program index temporary workspaces use one attempt ID"
    );
    Ok(matches.pop())
}
...
fn mark_cleanup_pending_path(
    ...
    match owner_start_ticks {
        ...
        None => ensure!(
            ...
            "unknown User-program index cleanup owner was not proven absent"
        ),
    }
...
fn validate_archive_file_identity(
    ...
    let actual = RegistryFileIdentity {
        ...
        "archive file identity differs from the user-program index manifest: {}",
        path.display()
    );
...
fn spawn_phase_with_pre_spawn(
    ...
    candidate.ensure_profile_bound_paths_do_not_alias_legacy(args)?;
    let worker_threads =
        u32::try_from(worker_threads).context("User-program index thread count is too large")?;
    let epoch_root = candidate.epoch_root(args);
    match fs::symlink_metadata(&epoch_root) {
        Ok(metadata) => ensure!(
            metadata.file_type().is_dir() && !metadata.file_type().is_symlink(),
            "User-program index epoch output root is not a real directory"
        ),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
        ...
        "User-program index epoch output root escaped its configured parent"
    );
    let now = unix_now();
    ...
    let executable_metadata = fs::metadata(&executable).with_context(|| {
        format!(
            "inspect User-program index executable {}",
            executable.display()
        )
    })?;
    ensure!(
        executable_metadata.file_type().is_file(),
        "User-program index executable is not a regular file"
    );
    ...
    candidate.recheck_authority_proofs("immediately before child spawn")?;
        ...
        "an exact User-program index worker already exists for epoch {}",
        candidate.epoch
    );
    ...
    command
        .args(&argv)
        .env_remove("BLOCKZILLA_FIREWATCH_ATTEMPT_ID")
        .env("BLOCKZILLA_USER_PROGRAM_INDEX_ATTEMPT_ID", &attempt_id)
        .stdin(Stdio::null())
    ...
    let start_ticks = match wait_for_spawned_identity(&marker, &mut child, pid) {
        ...
        Err(error) => {
            kill_pid_group(pid);
            let _ = child.wait();
            return Err(error).context("bind spawned User-program index process identity");
        }
    };
    ...
    if !process_matches_marker_pid(&starting_with_process, pid)? {
        terminate_pid_group(pid, start_ticks);
        let _ = child.wait();
        bail!("spawned User-program index process identity changed before publication")
    }
    if let Err(error) = publish_json_atomic(&candidate.workflow_path(args), &starting_with_process)
    {
        terminate_pid_group(pid, start_ticks);
        let _ = child.wait();
        return Err(error).context("publish starting User-program index process identity");
    }
    let owned_temp_path =
        match wait_for_owned_temp(args, candidate, phase, pid, &attempt_id, &mut child) {
            ...
            Err(error) => {
                terminate_pid_group(pid, start_ticks);
                let _ = child.wait();
                return Err(error).context("bind User-program index temporary workspace");
            }
        };
    ...
    if let Err(error) = publish_json_atomic(&candidate.workflow_path(args), &running) {
        ...
        return Err(error).context("publish spawned User-program index process identity");
    }
    Ok(ActiveChild {
        candidate: candidate.clone(),
        phase,
...
fn wait_for_spawned_identity(marker: &WorkflowMarker, child: &mut Child, pid: u32) -> Result<u64> {
    ...
    loop {
        if let Some(status) = child.try_wait()? {
            bail!("spawned User-program index process exited before identity binding: {status}");
        }
        if let Some(start_ticks) = process_start_ticks(pid) {
            if let Some(observed) = observed_start_ticks {
                ensure!(
                    observed == start_ticks,
                    "spawned User-program index PID changed during identity binding"
                );
            } else {
            ...
            Instant::now() < deadline,
            "spawned User-program index process did not expose its exact identity within 3 seconds"
        );
...
fn wait_for_owned_temp(
    ...
    let (parent, prefix) = match phase {
        Phase::TargetBuild | Phase::SourceControlBuild | Phase::CanonicalBuild => {
            ...
            let name = final_path
                .file_name()
                .context("User-program index output has no file name")?
                .to_string_lossy();
            (
                final_path
                    .parent()
                    .context("User-program index output has no parent")?
                    .to_path_buf(),
    ...
    for _ in 0..100 {
        ...
        for entry in fs::read_dir(&parent)? {
            ...
            matches.len() <= 1,
            "multiple temporary workspaces match one User-program index attempt"
        );
    ...
    bail!("User-program index child did not create its temporary workspace")
}
    ...
    for root_name in ["epochs", "canonical-epochs"] {
        ...
        for entry in fs::read_dir(&root)? {
            ...
            if let Some(start_ticks) = marker.process_start_ticks {
                    ...
                    find_marker_processes(&marker)?.is_empty(),
                    "an exact pidless User-program index attempt process appeared during cleanup"
                );
            }
            if cleanup_owned_temp_step(args, path, &marker, pid)? {
                if terminal_failure {
                    mark_failed_path(
                        &entry.path(),
                        marker,
                        "failed User-program index workspace cleanup is complete",
                    )?;
                } else {
                    mark_interrupted_path(
                        &entry.path(),
                        marker,
                        "stopped User-program index workspace cleanup is complete",
                    )?;
                }
            }
            return Ok(Some(format!(
                "cleaning a stopped User-program index workspace for epoch {epoch} during a safe idle window"
            )));
        }
    }
    Ok(None)
...
fn finish_child(args: &Args, mut child: ActiveChild, success: bool) -> Result<bool> {
        ...
        Err(error) => {
            ...
            if let Err(cleanup_error) = defer_child_cleanup(args, &child, true, message) {
                return Err(error).context(format!(
                    "durable User-program index cleanup deferral also failed: {cleanup_error:#}"
                ));
            }
            child.owned_temp_path = None;
            return Err(error);
    ...
    if let Err(error) = record_successful_eta_sample(args, &child) {
        tracing::warn!(
            ...
            %error,
            "User-program index phase completed, but its ETA sample could not be saved"
        );
    }
    Ok(clean_completion)
}
...
fn load_or_initialize_eta_history(args: &Args) -> Result<EtaHistory> {
    ...
    history
        .validate()
        .context("validate User-program index ETA history")?;
    changed |= ensure_eta_seed_groups(&mut history);
    history
        .validate()
        .context("validate seeded User-program index ETA history")?;
    if changed {
...
fn trim_eta_history(history: &mut EtaHistory) {
    ...
    history.samples.retain(|sample| {
        let count = group_counts
            ...
            .or_default();
        let keep = *count < user_program_index_controller_eta::MAX_SAMPLES_PER_GROUP;
        *count += 1;
        keep
    });
...
fn record_successful_eta_sample(args: &Args, child: &ActiveChild) -> Result<()> {
    let phase = eta_phase_for(&child.candidate, child.phase)
        .context("completed User-program index phase has no ETA calibration mapping")?;
    ...
    history
        .validate()
        .context("validate updated User-program index ETA history")?;
...
fn defer_child_cleanup(
    ...
    message: String,
        ...
        process_start_ticks(child.pid) != Some(child.start_ticks),
        "cannot defer cleanup while the User-program index child is live"
    );
    let marker_path = child.candidate.workflow_path(args);
    let marker: WorkflowMarker = read_bounded_json(&marker_path)?;
    ensure!(
        marker_matches_candidate(&marker, &child.candidate)
            && marker.pid == Some(child.pid)
            && marker.process_start_ticks == Some(child.start_ticks),
        "User-program index cleanup marker process binding changed"
    );
...
fn update_running_marker(args: &Args, child: &ActiveChild) -> Result<()> {
    child
        ...
        process_start_ticks(child.pid) == Some(child.start_ticks),
        "User-program index child PID identity changed"
    );
...
mod tests {
    ...
    fn test_active_child(candidate: Candidate, phase: Phase, worker_threads: u32) -> ActiveChild {
        ...
        ActiveChild {
            ...
            child,
            start_ticks: u64::MAX,
            log_path: PathBuf::from("/tmp/user-program-index-eta-test.log"),
            paused: false,
            pause_reason: None,
            owned_temp_path: None,
            io_sample: None,
    ...
    #[test]
    fn running_user_program_index_does_not_pause_for_unattributed_host_io() {
        assert_eq!(
            test_pressure(
    ...
    #[test]
    fn running_user_program_index_does_not_cancel_for_its_own_host_memory_pressure() {
        assert_eq!(
            test_pressure(
                &scheduler(4 * GIB, 1.1, 0.0),
                512 * GIB,
    ...
    #[test]
    fn paused_user_program_index_waits_for_host_io_to_recover() {
        assert!(matches!(
            test_pressure(
                &scheduler(4 * GIB, 0.0, 99.0),
                512 * GIB,
    ...
    #[test]
    fn scheduler_pause_and_active_lane_pause_user_program_index() {
    ...
    #[test]
    fn only_typed_runnable_scheduler_work_stops_user_program_index() {
        let mut status = scheduler(4 * GIB, 0.0, 0.0);
    ...
    fn parity_output_is_exact_and_rejects_duplicates_or_mismatch() {
        let root = std::env::temp_dir().join(format!(
            "user-program-index-controller-test-{}",
            std::process::id()
        ));
