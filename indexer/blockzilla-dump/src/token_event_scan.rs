...
pub enum TokenEventScanError {
    ...
    #[error(transparent)]
    Scan(#[from] blockzilla_model::Error),
}
...
mod tests {
    ...
    impl ArchiveInstructionSource for FixtureSource {
        ...
        fn scan_ordered(
            &mut self,
            request: &ScanRequest,
            sink: &mut dyn blockzilla_model::BlockSink,
                ...
                CanonicalBlock {
                    ...
                    transactions: vec![CanonicalTransaction {
                        ...
                        token_balance_coverage:
                            blockzilla_model::TokenBalanceCoverage::NotRequested,
    ...
    fn source_identity_and_verification_policy_fail_closed() {
        ...
        assert!(matches!(
            ...
            Err(TokenEventScanError::Scan(
                blockzilla_model::Error::InvalidRequest(_)
            ))
        ));
