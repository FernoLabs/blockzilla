...
Keep deployment-specific hosts, journal IDs, and paths in the operator's secret
configuration, not in this repository. Recheck every value before every run.

```text
spool root     /srv/blockzilla/replication-receiver
cluster        solana-mainnet
origin         <source-origin>
source         shred-reader-loopback
journal        <32-hex-journal-id>
progress WAL   <journal>/receiver-progress.wal
audit binary   /usr/local/bin/shred-epoch-audit
```

The run records and later rechecks the actual audit-binary hash rather than
trusting a historical deployment value.

## 1. Open an operator shell and set constants

Run the following in one persistent Bash session on the NAS after the source deployment is
healthy. Connect using the deployment's configured SSH identity, start `tmux new -s
...
ROOT=${ROOT:?export the replication receiver root}
CLUSTER=${CLUSTER:-solana-mainnet}
ORIGIN=${ORIGIN:?export the exact source origin}
SOURCE=${SOURCE:-shred-reader-loopback}
JID=${JID:?export the exact 32-hex journal ID}
...
AUDIT_ROOT=${AUDIT_ROOT:-/var/lib/blockzilla/audits}
AUDIT_DIR="$AUDIT_ROOT/fresh-raw-$RUN"
