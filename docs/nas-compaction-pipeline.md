...
- pauses one newest managed worker per reconciliation pass when I/O PSI reaches
  the pause threshold, CPU load reaches its ceiling, or available memory falls below
...
- resumes at most one worker per cooldown only after I/O PSI falls to the lower
  ...
  `memory reserve + 2 × memory guard` and CPU load has recovered;
- reports stale or non-advancing live ingest as an independent operational
  fault, but never pauses historical compaction for it; the live producer's
  supervisor/watchdog owns reconnect and replay recovery;
...
Live ingest health is advisory to the historical compaction controller. A
stale or non-advancing live slot is repaired by the producer supervisor and is
reported as a live-capture fault, but it never sets the compaction throughput
target to zero or pauses historical workers. Memory, CPU, I/O-pressure, and an
explicit archive-device throughput ceiling remain independent hard admission
guards. On controller restart, trusted automatic pauses from the deprecated
live-ingest guard are resumed; manual pauses remain stopped and unverifiable
process identities still fail closed.

## Live capture and repair gate

The target ownership boundary is recorder-only ingest plus finite scheduler
work:

```text
Yellowstone -> record-grpc-raw -> durable raw WAL
                                  |
                                  v
                 live_materialize:<source_id>:<epoch>
                                  |
                                  v
                     repair/audit -> registry -> MPHF -> hot rewrite
```

`record-grpc-raw` owns connection liveness, inclusive reconnect overlap, and
durable event append only. Block conversion, PoH extraction, block/signature/
pubkey indexes, registry construction, and compaction are bounded scheduler
tasks. Transport health is reported independently and can never reduce
historical compaction capacity to zero.

Until the raw committed-prefix reader and a full shadow epoch pass, the NAS
continues using `capture-grpc`. The current replay path takes the recorder's
exclusive writer lock, so it is not safe to schedule against an active spool.
The replacement reader must snapshot a durable journal prefix, consume only
complete verified frames from that prefix, never mutate the spool, and
materialize a proven closed epoch through a hidden staging directory. It
publishes a source-bound completion receipt last and atomically renames before
the existing live registry/MPHF/hot-rewrite tasks can consume the result.
