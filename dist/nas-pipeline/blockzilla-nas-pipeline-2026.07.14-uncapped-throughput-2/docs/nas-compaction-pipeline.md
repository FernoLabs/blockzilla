...
- independent legacy registry-reuse ranges may run concurrently inside static
  CPU, I/O, memory, and disk budgets, with optional adaptive pause/resume;
...
Independent contiguous range heads may run together. The configured legacy
concurrency is only a hard ceiling: effective capacity is the minimum of that
ceiling, the CPU budget divided by the per-worker CPU reservation, and the I/O
budget divided by the per-worker I/O reservation. Candidate-specific memory
and free-disk admission can reduce it further. A paused worker still counts in
all of these limits and retains its full remaining memory reservation.
...
- pauses at most one newest managed worker per cooldown when I/O PSI reaches
  the pause threshold or available memory falls below
  `memory reserve + memory guard`;
- resumes at most one worker per cooldown only after I/O PSI falls to the lower
  ...
  `memory reserve + 2 × memory guard`;
- never goes below `--legacy-compact-min-running` total running legacy lanes;
- starts at most one additional legacy lane per cooldown after recovery;
...
```text
legacy hard concurrency             4
legacy CPU budget                   8 cores at 1 core/worker
legacy I/O budget                   360 MiB/s at 120 MiB/s/worker (3 effective lanes)
adaptive pause                      enabled
minimum running legacy lanes        2
free-memory reserve                 1,536 MiB
adaptive memory guard               512 MiB
I/O PSI full avg10 pause / resume   15% / 8%
adaptive action cooldown            60 seconds
```
