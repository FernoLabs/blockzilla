...
| Nonempty final metadata, MPHF, manifest, and required sidecars; no scan marker | Historical epoch complete |
...
The package intentionally does not replace the production `blockzilla` binary.
Point `BLOCKZILLA_BIN` at the separately validated low-memory binary. This keeps
the control-plane rollout reversible and avoids changing an active live capture
or finalizer process.
...
Edit `/home/ach/dev/blockzilla-pipeline/config/nas-pipeline.env` and pin
`BLOCKZILLA_BIN` to the validated binary, for example:
