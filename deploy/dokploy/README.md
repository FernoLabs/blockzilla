...
3. On the deployment host, create the Yellowstone and Telegram tokens as
   one-line files owned by `root:10001` with mode `0640`. [Compose file
   secrets](https://docs.docker.com/reference/compose-file/services/#secrets)
   use read-only bind mounts for file sources and cannot remap ownership, so
   group `10001` is required by the non-root container process:

   ```sh
   install -o root -g 10001 -m 0640 /dev/null /etc/blockzilla/yellowstone-x-token
   install -o root -g 10001 -m 0640 /dev/null /etc/blockzilla/telegram-bot-token
   sudoedit /etc/blockzilla/yellowstone-x-token
   sudoedit /etc/blockzilla/telegram-bot-token
   ```

   Never put real tokens in Git, Dokploy variables, build arguments, or logs.
   [`yellowstone-x-token.example`](yellowstone-x-token.example) and
   [`telegram-bot-token.example`](telegram-bot-token.example) show the file
   shapes. The `*_HOST_FILE` settings may point to different absolute host
   paths. Inside the container the credentials exist only at
   `/run/secrets/grpc_x_token` and `/run/secrets/telegram_bot_token`, not as
   environment values. Compose configuration can reveal the host file paths,
   and anyone with Docker/Dokploy administrative access can still read mounted
   secrets, so restrict that access.
