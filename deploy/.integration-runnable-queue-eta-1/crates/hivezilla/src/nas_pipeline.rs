...
#[derive(Debug, Deserialize)]
struct PublishedSourceMaterializedMarker {
    version: u16,
    state: String,
    canonical: bool,
    publication_ready: bool,
...
fn validate_repair_compacted_sources(
    ...
    let source_marker: PublishedSourceMaterializedMarker =
...
fn active_repair_progress(
    ...
    let repair_blockzilla_bin = config
        .repair_blockzilla_bin
        .as_deref()
        .unwrap_or(&config.blockzilla_bin);
    let materialized_root = config.state_root.join("live-repair-materialized");
    let materialized_output = materialized_root.join(format!("epoch-{epoch}"));
    let archive_output = config.archive_root.join(format!("epoch-{epoch}"));
    let candidates = [
        (
            materialized_root
                .join(format!(".epoch-{epoch}.repair-materialize-stage"))
    ...
    candidates
        ...
        .filter_map(|(path, kind)| {
            ...
            let reported_rss_bytes = read_reported_progress_rss_bytes(&path);
            let updated = progress
                .updated_unix_secs
                .or_else(|| modified_unix_secs(&path))?;
            ...
            progress.rss_bytes = process_tree_rss_bytes(pid).or(reported_rss_bytes);
            progress.peak_rss_bytes = process_peak_rss_bytes(pid)
                .or(progress.peak_rss_bytes)
                .map(|peak| peak.max(progress.rss_bytes.unwrap_or(0)));
            if progress.progress_pct.is_none() && progress.blocks_total > 0 {
                progress.progress_pct = Some(
                    progress.blocks_done.min(progress.blocks_total) as f64
                        / progress.blocks_total as f64
                        * 100.0,
                );
            }
            Some(progress)
...
fn read_reported_progress_rss_bytes(path: &Path) -> Option<u64> {
    const MAX_PROGRESS_BYTES: u64 = 1024 * 1024;
    let file = File::open(path).ok()?;
    let value: Value =
        serde_json::from_reader(BufReader::new(file.take(MAX_PROGRESS_BYTES))).ok()?;
    json_u64(&value, &["rss_bytes"])
}
...
fn classify_live_capture(
    ...
    match read_published_repair_bundle(&config.live_root, &capture_dir) {
        Ok(Some(bundle)) => {
            ...
            let active_progress = active_repair_progress(config, &capture_dir, bundle.epoch, now);
...
fn legacy_tuning_enabled(config: &NasPipelineConfig) -> bool {
...
fn legacy_tuner_bootstrap_lanes(config: &NasPipelineConfig, context: LegacyTunerContext) -> usize {
    match context {
...
mod tests {
    ...
    fn write_test_repair_compacted(
        config: &NasPipelineConfig,
        bundle: &Path,
        epoch: u64,
    ) -> PathBuf {
    ...
    #[test]
    fn valid_degraded_repair_compaction_is_packaged_and_keeps_grouping() {
    ...
    #[test]
    fn symlinked_degraded_repair_marker_is_never_accepted() {
        ...
        assert!(
            bundle
                ...
                .as_deref()
                .unwrap()
                .contains("not a regular degraded-compaction marker")
        );
        fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn repair_process_argv_matching_is_byte_exact() {
        let root = temp_root("repair-process-argv");
        let bin = root.join("blockzilla");
        let capture = root.join("live/epoch-1000-repair-view");
        let materialized = root.join("state/live-repair-materialized/epoch-1000");
