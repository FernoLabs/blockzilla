...
struct NasPipelineArgs {
    ...
    /// Optional newer Blockzilla binary used only to identify repair materialization/compaction.
    /// Historical scheduler adoption continues to use --blockzilla-bin byte-for-byte.
    #[arg(long)]
    ...
    /// Deprecated fixed lane ceiling. Zero enables uncapped adaptive probing;
    /// CPU, memory, disk, PSI, and useful-throughput guards still apply.
    #[arg(long, default_value_t = 0)]
    legacy_compact_concurrency: usize,
    /// Optional compatibility ceiling for compact lanes while a finalizer is
    /// active. In adaptive mode zero is uncapped; in fixed mode zero preserves
    /// strict finalizer exclusivity.
    #[arg(long, default_value_t = 0)]
    legacy_compact_finalizer_overlap: usize,
    /// Deprecated per-lane CPU estimate retained for status compatibility. It
    /// is not converted into a lane-count ceiling.
    #[arg(long, default_value_t = 1)]
    legacy_compact_cpu_cores_per_worker: u64,
    /// Global one-minute load ceiling used by adaptive pause/resume.
    ...
    /// Optional measured archive-device throughput ceiling. Zero disables the
    /// ceiling; this is never divided into a fixed number of worker slots.
    #[arg(long, default_value_t = 0)]
    ...
    /// Initial/baseline lane target. Hard resource guards may pause below it.
    #[arg(long, default_value_t = 1)]
    ...
    /// Pause when Linux IO PSI full avg10 reaches this percentage.
    #[arg(long, default_value_t = 20.0)]
    legacy_compact_io_pause_full_avg10: f64,
    /// Resume only when Linux IO PSI full avg10 falls to this percentage.
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::NasPipeline(args) => {
            ...
            let legacy_effective_capacity = if args.legacy_compact_concurrency == 0 {
                usize::MAX
            } else {
                args.legacy_compact_concurrency
            };
            anyhow::ensure!(
            ...
            anyhow::ensure!(
                args.legacy_compact_finalizer_overlap <= legacy_effective_capacity,
                "--legacy-compact-finalizer-overlap must not exceed effective legacy capacity ({legacy_effective_capacity})"
            );
            anyhow::ensure!(
            ...
            run_nas_pipeline(NasPipelineConfig {
                ...
                legacy_compact_finalizer_overlap: args.legacy_compact_finalizer_overlap,
