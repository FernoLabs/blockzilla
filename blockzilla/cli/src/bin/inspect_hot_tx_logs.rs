...
use blockzilla_archive_v2::{
    ...
    ArchiveV2HotTxRow, deserialize_archive_v2_hot_block_blob, read_archive_v2_block_access_index,
...
use blockzilla_compact::{
    ...
    CompactTokenBalance, CompactTransactionError, LogEvent,
};
...
fn main() -> Result<()> {
    ...
    let index = read_archive_v2_hot_block_index(
        &blockzilla_archive_v2::archive_v2_hot_index_path(&blocks_path),
    )?;
    let row = index
        .rows
        .iter()
