...
use blockzilla_read_car::{
    DEFAULT_USER_PROGRAM_INDEX_WALLET, OutputFacts, RunFacts, WorkloadSource, create_output,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let request = user_program_index_scan_request(base_request).with_required_signer(wallet);

    let mut sink = UserProgramIndexSink::new(create_output(&arguments.output)?, wallet)?;
