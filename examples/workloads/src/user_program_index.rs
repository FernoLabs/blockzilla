...
pub const USER_PROGRAM_INDEX_HEADER_BYTES: usize = 44;
pub const USER_PROGRAM_INDEX_RECORD_BYTES: usize = 64;
// Keep the legacy wire identifiers so the workload rename does not change
// saved output bytes, schema validation, or cross-format parity.
const USER_PROGRAM_INDEX_SCHEMA: &str = "blockzilla-example-firewatch-wallet-program/v1";
const USER_PROGRAM_INDEX_MAGIC: [u8; 8] = *b"BZFWAL01";

/// The signer transaction can be successful or failed.
pub const USER_PROGRAM_INDEX_COVERAGE_UNKNOWN_EXECUTION: u8 = 1 << 0;
/// Some outer reached-program evidence can be absent from a successful transaction.
pub const USER_PROGRAM_INDEX_COVERAGE_INCOMPLETE_INSTRUCTIONS: u8 = 1 << 1;
/// Some CPI reached-program evidence can be absent from a successful transaction.
pub const USER_PROGRAM_INDEX_COVERAGE_INCOMPLETE_CPI: u8 = 1 << 2;

/// Select signer, status, and program identities without payload bytes.
pub fn user_program_index_scan_request(request: ScanRequest) -> ScanRequest {
    request
...
pub struct UserProgramIndexReport {
    pub blocks_seen: u64,
...
pub struct UserProgramIndexSink<W> {
    wallet: [u8; 32],
...
impl<W: Write> UserProgramIndexSink<W> {
    pub fn new(writer: W, wallet: [u8; 32]) -> Result<Self> {
        let header = target_header(
            USER_PROGRAM_INDEX_MAGIC,
            USER_PROGRAM_INDEX_RECORD_BYTES as u32,
            wallet,
        );
        Ok(Self {
            wallet,
    ...
    pub fn process_block(&mut self, block: BlockView<'_>) -> Result<()> {
        ...
        increment(&mut self.blocks_seen, "User program index block")
    }

    pub fn process_transaction(&mut self, transaction: TransactionView<'_>) -> Result<()> {
        self.order.observe("User program index", transaction)?;
        increment(
            &mut self.transactions_seen,
            "User program index transaction",
        )?;
        ...
        increment(
            ...
            "User program index signer transaction",
        )?;
        ...
        match transaction.header.status {
            ExecutionStatus::Failed => {
                increment(
                    ...
                    "User program index failed signer transaction",
                )?;
            }
            ExecutionStatus::Unknown(_) => {
                increment(
                    &mut self.unknown_execution_signer_transactions,
                    "User program index unknown-execution signer transaction",
                )?;
                self.coverage
                    .observe(transaction, USER_PROGRAM_INDEX_COVERAGE_UNKNOWN_EXECUTION)?;
            }
    ...
    fn process_successful(&mut self, transaction: TransactionView<'_>) -> Result<()> {
        increment(
            ...
            "User program index successful signer transaction",
        )?;
        ...
        if !matches!(
            ...
            reason_bits |= USER_PROGRAM_INDEX_COVERAGE_INCOMPLETE_INSTRUCTIONS;
            increment(
                &mut self.incomplete_instruction_transactions,
                "User program index incomplete-instruction transaction",
            )?;
        }
        if !matches!(transaction.header.cpi_coverage, CpiCoverage::Complete) {
            reason_bits |= USER_PROGRAM_INDEX_COVERAGE_INCOMPLETE_CPI;
            increment(
                &mut self.incomplete_cpi_transactions,
                "User program index incomplete-CPI transaction",
            )?;
        }
        for instruction in transaction.instructions {
            let program = instruction.program_id.ok_or_else(|| {
                crate::Error::InvalidInput(
                    "User program index needs program keys for successful signer transactions"
                        .into(),
                )
            })?;
        ...
        add(
            ...
            u64::try_from(transaction.instructions.len()).map_err(|_| {
                crate::Error::CounterOverflow("User program index reached instruction")
            })?,
            "User program index reached instruction",
        )?;
        if reason_bits != 0 {
    ...
    pub fn finish(self) -> Result<FinishedOutput<W, UserProgramIndexReport>> {
        let Self {
            wallet,
            mut output,
            coverage,
...
impl<W: Write> BlockSink for UserProgramIndexSink<W> {
...
mod tests {
    ...
    fn writes_sorted_distinct_programs_from_successful_signer_transactions() {
        ...
        let mut sink = UserProgramIndexSink::new(Vec::new(), wallet).unwrap();
        sink.process_block(source.as_view()).unwrap();
        let finished = sink.finish().unwrap();

        assert_eq!(&finished.writer[..8], b"BZFWAL01");
        assert_eq!(
            finished.report.output.schema,
            "blockzilla-example-firewatch-wallet-program/v1"
        );
        ...
        assert_eq!(
            ...
            (USER_PROGRAM_INDEX_HEADER_BYTES + 2 * USER_PROGRAM_INDEX_RECORD_BYTES) as u64
        );
        let first = &finished.writer[USER_PROGRAM_INDEX_HEADER_BYTES
            ..USER_PROGRAM_INDEX_HEADER_BYTES + USER_PROGRAM_INDEX_RECORD_BYTES];
        let second =
            &finished.writer[USER_PROGRAM_INDEX_HEADER_BYTES + USER_PROGRAM_INDEX_RECORD_BYTES..];
    ...
    #[test]
    fn omitting_instruction_accounts_preserves_user_program_index_output() {
        let wallet = [0x11; 32];
    ...
    fn request_keeps_partial_historical_coverage_visible() {
        let request = user_program_index_scan_request(ScanRequest::all());
        ...
        assert!(matches!(
            ...
            blockzilla_model::InstructionDataRequirement::None
        ));
    }
}
