...
These four small programs show one job each. They use the Indexer V3
SDK directly and do not select an archive format at run time.

- [`read-indexer-v3-slot-hours`](src/bin/read-indexer-v3-slot-hours.rs) reads
  every block and transaction and counts recorded inner instructions in
  slot-derived, approximate one-hour windows.
- [`read-indexer-v3-usdc`](src/bin/read-indexer-v3-usdc.rs) writes recorded
  USDC pre- and post-token balances.
- [`read-indexer-v3-pumpfun`](src/bin/read-indexer-v3-pumpfun.rs) writes
  transactions that call the Pump.fun program directly or through recorded
  CPI.
- [`read-indexer-v3-firewatch`](src/bin/read-indexer-v3-firewatch.rs) writes
...
```console
cargo build --release --locked -p blockzilla-read-indexer-v3 \
  --bin read-indexer-v3-slot-hours \
  --bin read-indexer-v3-usdc \
  --bin read-indexer-v3-pumpfun \
  --bin read-indexer-v3-firewatch
```
...
- [`read-indexer-v3`](src/main.rs) is the ordered reader benchmark.
- [`read-indexer-v3-transactions`](src/bin/read-indexer-v3-transactions.rs)
  ...
  report fields.
- [`read-indexer-v3-usdc-instructions`](src/bin/read-indexer-v3-usdc-instructions.rs)
...
[Indexer V3 SDK README](../../crates/blockzilla-indexer-v3-read-sdk/README.md).
