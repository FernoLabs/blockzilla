...
use blockzilla_indexer_v3_read_sdk::ArchiveInstructionSource;
use blockzilla_read_indexer_v3::{
    ReadProfile, arguments, finish_archive, open_archive, output_file, scan_request,
};

fn main() -> Result<(), Box<dyn Error>> {
    let arguments = arguments("read-indexer-v3-transactions")?;
    ...
    println!(
        ...
        match &arguments.source {
            blockzilla_read_indexer_v3::Source::Network { .. } => "network",
            blockzilla_read_indexer_v3::Source::LocalSplit { .. } => "local-split",
        },
...
fn transaction_request(
    request: blockzilla_indexer_v3_read_sdk::ScanRequest,
) -> blockzilla_indexer_v3_read_sdk::ScanRequest {
    request
...
fn subtract_transport(
    later: blockzilla_indexer_v3_read_sdk::IndexerV3TransportReceipt,
    earlier: blockzilla_indexer_v3_read_sdk::IndexerV3TransportReceipt,
) -> blockzilla_indexer_v3_read_sdk::IndexerV3TransportReceipt {
    blockzilla_indexer_v3_read_sdk::IndexerV3TransportReceipt {
        kind: later.kind,
...
fn transport_name(kind: blockzilla_indexer_v3_read_sdk::IndexerV3TransportKind) -> &'static str {
    match kind {
        blockzilla_indexer_v3_read_sdk::IndexerV3TransportKind::HttpCached => "http-cached",
        blockzilla_indexer_v3_read_sdk::IndexerV3TransportKind::LocalSplit => "local-split",
    }
}
...
mod tests {
    ...
    fn parses_a_local_parallel_transaction_dump() {
        let arguments = blockzilla_read_indexer_v3::arguments_from(
            "read-indexer-v3-transactions",
            None,
            [
                "local-split",
                "/ledger",
