...
impl ApproximateHourCounts {
    ...
    fn print(&self) {
        ...
        for (index, bucket) in self.buckets.iter().enumerate() {
            let offset = (index as u64).saturating_mul(SLOTS_PER_APPROXIMATE_HOUR);
            let start_slot = self.first_slot.saturating_add(offset);
            let end_slot = start_slot
                .saturating_add(SLOTS_PER_APPROXIMATE_HOUR)
                .min(self.end_slot);
            println!(
...
impl BlockSink for ApproximateHourCounts {
    fn visit_block(&mut self, block: BlockView<'_>) -> QueryResult<()> {
        ...
        // The parallel SDK must deliver this callback in strict ledger order.
        ...
        for transaction in block.transaction_views() {
            ...
            increment(
                ...
                u64::try_from(inner).map_err(|error| QueryError::sink(io::Error::other(error)))?,
            )?;
        }
...
fn increment(count: &mut u64, amount: u64) -> QueryResult<()> {
    *count = count
        .checked_add(amount)
        .ok_or_else(|| QueryError::sink(io::Error::other("count overflow")))?;
    Ok(())
}
