...
const USAGE: &str = "usage:\n  read-indexer-v3 <worker-origin> <epoch> <absolute-cache-root> [max-blocks]\n  read-indexer-v3 local-split <ledger-root> <retained-sidecar-root> <epoch> <candidate-id> [max-blocks]";
...
fn open_archive(arguments: &Arguments) -> blockzilla_indexer_v3_read_sdk::Result<IndexerV3Archive> {
...
fn benchmark_request(range: blockzilla_indexer_v3_read_sdk::ScanRange) -> ScanRequest {
...
struct ResultView<'a> {
    ...
    verification: blockzilla_indexer_v3_read_sdk::SourceVerification,
