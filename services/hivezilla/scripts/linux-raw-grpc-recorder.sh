...
BIN=${BLOCKZILLA_RAW_BIN:-/usr/local/bin/hivezilla}
...
PROGRESS_RESTART_DELAY_SECS=${BLOCKZILLA_RAW_PROGRESS_RESTART_DELAY_SECS:-0}
...
ORIGIN_NODE_ID=${BLOCKZILLA_RAW_ORIGIN_NODE_ID:-source-node-01}
SOURCE_ID=${BLOCKZILLA_RAW_SOURCE_ID:-grpc-raw-source-backup}

# The legacy mode keeps one ever-growing dedicated-volume spool. Generation
# modes store one bounded live generation at a stable path and expose immutable
# sealed generations only after a seeded successor has become active. The
# local-generations mode deliberately has no uploader or retention worker.
...
case "$OBJECT_STORE" in
  none)
    OBJECT_STORE_HUMAN_NAME='local storage'
    OBJECT_STORE_DEFAULT_CREDENTIALS_FILE=
    OBJECT_STORE_DEFAULT_REMOTE_PREFIX=
    ;;
  r2)
...
validate_data_paths() {
  case "$CACHE_MODE" in
    legacy|b2-generations|local-generations) ;;
    *)
      echo "BLOCKZILLA_RAW_CACHE_MODE must be legacy, b2-generations, or local-generations" >&2
      return 1
      ;;
  esac
...
validate_object_store() {
  case "$OBJECT_STORE:$CACHE_MODE" in
    backblaze:legacy|backblaze:b2-generations|r2:legacy|r2:b2-generations|none:local-generations) ;;
    *)
      echo "BLOCKZILLA_RAW_OBJECT_STORE must be backblaze or r2; local-generations requires none" >&2
      return 1
      ;;
  esac
...
# These defaults keep a quiet connection alive with adaptive HTTP/2 flow-control
...
export BLOCKZILLA_GRPC_CONNECT_TIMEOUT_SECS=${BLOCKZILLA_GRPC_CONNECT_TIMEOUT_SECS:-10}
export BLOCKZILLA_GRPC_HTTP2_ADAPTIVE_WINDOW=${BLOCKZILLA_GRPC_HTTP2_ADAPTIVE_WINDOW:-true}
export BLOCKZILLA_GRPC_HTTP2_KEEP_ALIVE_INTERVAL_SECS=${BLOCKZILLA_GRPC_HTTP2_KEEP_ALIVE_INTERVAL_SECS:-30}
export BLOCKZILLA_GRPC_HTTP2_KEEP_ALIVE_TIMEOUT_SECS=${BLOCKZILLA_GRPC_HTTP2_KEEP_ALIVE_TIMEOUT_SECS:-30}
...
notify_parent_supervisor() {
  notification_kind=$1
  if [ -z "${BLOCKZILLA_SUPERVISOR_NOTIFY_FILE:-}" ] \
    || [ -z "${BLOCKZILLA_SUPERVISOR_NOTIFY_TOKEN:-}" ]
  then
    return 0
  fi
  "$BIN" notify-supervisor "$notification_kind" >/dev/null
}
...
monitor_disk_alerts() {
  if ! monitor_free_bytes=$(available_bytes); then
    raise_alert disk_check_failed ERROR \
      "Status: source host free space is unavailable. Backup safety cannot be confirmed.
Storage: Unknown.
Action: Check the source host disk."
    return 0
  fi
  clear_alert disk_check_failed \
    "Status: source host disk checks are working again.
  ...
  if [ "$CACHE_MODE" = b2-generations ]; then
    ...
    if [ "$disk_free_bytes" -lt "$MIN_FREE_BYTES" ]; then
      ...
      raise_alert generation_backlog CRITICAL \
        "Status: source host disk is full. Backup is paused; saved data is safe.
Storage: $(human_decimal_bytes "$disk_free_bytes") free; backup needs $(human_decimal_bytes "$MIN_FREE_BYTES"). $pipeline_disk_context
Action: Nothing is deleted without Blockzilla confirmation. Add source host disk space and restore Blockzilla sync."
      return 0
    fi
...
Action: Free source host disk space or restore $OBJECT_STORE_HUMAN_NAME uploads. Automatic retry is on."
  elif [ "$disk_free_bytes" -lt "$DISK_WARN_FREE_BYTES" ]; then
    raise_alert disk_space WARNING \
      "Status: The source host disk is getting low, but it still has room for recording.
...
Data: source host keeps all unconfirmed blocks. Indexing is a separate check.
  ...
  while kill -0 "$monitored_pid" 2>/dev/null; do
    ...
    if validate_data_volume true >/dev/null 2>&1; then
      ...
      clear_alert volume_invalid \
        "Status: The source host backup disk is available again.
...
Action: None."
      ...
      raise_alert volume_invalid CRITICAL \
        "Status: The source host backup disk is unavailable. Backup is paused.
Data: Nothing was deleted.
Action: Check the source host volume mount."
      break
    fi
  done
}
...
raw_child_report_backoff_seconds() {
  ...
  elif report_frames_written=$(raw_child_report_uint "$1" frames_written) \
    && [ -n "$report_frames_written" ] \
    && [ "$report_frames_written" -gt 0 ] 2>/dev/null
  then
    # A durable append proves the provider accepted this session. Reconnect
    # quickly so a transient stream reset does not become a multi-slot hole.
    printf '%s\n' "$PROGRESS_RESTART_DELAY_SECS"
  else
    # A session that never crossed the durability boundary keeps the more
    # conservative delay, preventing a tight loop during a provider outage.
    printf '%s\n' "$RESTART_DELAY_SECS"
  fi
}
...
load_replay_recovery_floor() {
  ...
  if [ -n "$REPLAY_PROVIDER_AVAILABLE_SLOT" ]; then
    REPLAY_GAP_CARRY_FILE=replay-gap-$replay_recovery_anchor-$replay_recovery_requested-$REPLAY_PROVIDER_AVAILABLE_SLOT-$replay_recovery_floor.json
  else
    REPLAY_GAP_CARRY_FILE=replay-gap-$replay_recovery_anchor-$replay_recovery_requested-$replay_recovery_floor.json
  fi
...
raise_generation_spill_gate_alert() {
  ...
  spill_gate_action="Action: Check the source host disk and recorder service. Automatic retry is on."
  case "$spill_gate_failure" in
    queue)
      ...
      ;;
    free_space)
      spill_gate_status="Status: Cannot read source host free space. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."
      ;;
    capacity)
      spill_gate_status="Status: Cannot read source host disk size. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."
      ;;
    policy)
      spill_gate_status="Status: Storage safety check failed. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."
      ;;
    blockzilla_ack)
      spill_gate_status="Status: Blockzilla has not confirmed these blocks. source host cleanup is paused; saved data is safe."
      spill_gate_action="Action: Nothing was deleted. Restore Blockzilla sync or add source host disk space. $OBJECT_STORE_HUMAN_NAME uploads continue."
      ;;
    *)
      spill_gate_status="Status: Storage safety check failed. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."
      ;;
...
generation_backlog_local_incident_active() {
  ...
  case "$gate_incident_marker" in
    ...
    "Status: Cannot read source host free space. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."|\
    "Status: Cannot read source host disk size. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."|\
    "Status: Storage safety check failed. $OBJECT_STORE_HUMAN_NAME upload is paused; saved data is safe."|\
    'Status: Blockzilla has not confirmed these blocks. source host cleanup is paused; saved data is safe.'|\
    'Status: Cannot read local backup files. Backblaze upload is paused; saved data is safe.'|\
    'Status: Cannot read source host free space. Backblaze upload is paused; saved data is safe.'|\
    'Status: Cannot read source host disk size. Backblaze upload is paused; saved data is safe.'|\
    ...
    'Status: Cannot read source host free space. Upload cleanup is paused; saved data is safe.'|\
    'Status: Cannot read source host disk size. Upload cleanup is paused; saved data is safe.'|\
  ...
  case "$gate_incident_marker" in
    'Cause: source host could not inspect the sealed-generation queue safely.'|\
    'Cause: source host could not measure free space on the recorder filesystem.'|\
    'Cause: source host could not measure the recorder filesystem capacity.'|\
    'Cause: source host could not validate the disk spill watermarks from the measured filesystem values.'|\
    'Cause: The disk-first source host cache reached its safety floor before Backblaze spill cleanup recovered headroom.')
      return 0
      ;;
    *) return 1 ;;
  esac
      ...
      if [ "$generation_policy_ready" = true ]; then
        ...
        if [ "$generation_upload_failed" = true ]; then
          ...
          if [ "$generation_space_valid" != true ]; then
            ...
            pipeline_capture="source host still has room for recording; saved data is safe."
          fi
...
Storage: $(human_decimal_bytes "$generation_free_bytes") free on source host.
Action: None."
        elif [ "$generation_space_valid" = true ] \
          && [ "$generation_spill_active" = true ] \
          && [ "$generation_backlog" -gt 0 ] \
        ...
        then
          ...
          rm -f "$(alert_file generation_backlog handoff)" 2>/dev/null || true
          raise_alert generation_backlog "$pipeline_level" \
            "Status: $OBJECT_STORE_HUMAN_NAME upload finished, but source host space did not increase. $pipeline_capture
...
r2_receipt_ledger_bytes() {
  ...
  r2_receipt_state=$(r2_receipt_directory_state) || return 1
  case "$r2_receipt_state" in
    empty)
      # A fresh provider-bound receipt ledger has no remote committed payload
      # yet. Do not require a synthetic chain head merely to account for zero.
      printf '%s\n' 0
      return 0
      ;;
    chained) ;;
...
if ! notify_parent_supervisor ready; then
  echo "$(timestamp) raw_recorder supervisor_readiness_failed" >&2
  exit 1
fi
    ...
    if [ -n "$REPLAY_MIN_RESUME_SLOT" ]; then
      ...
      if [ -n "$REPLAY_PROVIDER_AVAILABLE_SLOT" ]; then
        raise_alert provider_replay_gap WARNING \
          "Status: Live gRPC capture has not resumed yet.
Data: Saved through slot $REPLAY_RECOVERY_ANCHOR_SLOT. Missing slots $((REPLAY_RECOVERY_ANCHOR_SLOT + 1))-$((REPLAY_MIN_RESUME_SLOT - 1)).
Action: Retrying automatically from slot $REPLAY_MIN_RESUME_SLOT; recovery is confirmed only after a new block is saved."
      else
        raise_alert provider_replay_gap WARNING \
          "Status: Live gRPC capture has not resumed yet.
Data: Saved through slot $REPLAY_RECOVERY_ANCHOR_SLOT. Missing slots $((REPLAY_RECOVERY_ANCHOR_SLOT + 1))-$((REPLAY_MIN_RESUME_SLOT - 1)).
Action: Retrying automatically from slot $REPLAY_MIN_RESUME_SLOT; recovery is confirmed only after a new block is saved."
    ...
    write_state volume_invalid
    raise_alert volume_invalid CRITICAL \
      "Status: The source host backup folder is unavailable. Backup is paused.
Data: Nothing was deleted.
Action: Check the source host volume mount."
...
Action: None."
  ...
  if [ -n "$REPLAY_MIN_RESUME_SLOT" ]; then
    set -- "$@" \
      --min-resume-slot "$REPLAY_MIN_RESUME_SLOT" \
      --pending-replay-gap-file "$REPLAY_GAP_CARRY_FILE"
  fi
  ...
  recorder_retry_seconds=$(raw_child_report_backoff_seconds "$CHILD_REPORT_FILE")
...
Data: Everything saved earlier is safe.
Action: Check the source host monitoring folder."
  fi
  if [ "$status" -eq 0 ] \
    && [ "$exit_reason" = upstream_access_blocked ]
  then
...
Action: None."
      ...
      raise_alert provider_replay_gap WARNING \
        "Status: Live gRPC capture has not resumed yet.
Data: Saved through slot $REPLAY_GAP_ANCHOR_SLOT. Missing slots $((REPLAY_GAP_ANCHOR_SLOT + 1))-$((REPLAY_GAP_SELECTED_RESUME_SLOT - 1)).
Action: Retrying automatically from slot $REPLAY_GAP_SELECTED_RESUME_SLOT; recovery is confirmed only after a new block is saved."
      write_capture_progress_state
...
Action: Check the source host volume mount."
    ...
    raise_recorder_restart_alert \
      "Recorder exited; status=$status reason=$exit_reason last_slot=$last_slot journal_age_seconds=$journal_age. Restarting after ${recorder_retry_seconds}s."
  fi
  if [ -s "$CHILD_REPORT_FILE" ]; then
    sed -n '1,200p' "$CHILD_REPORT_FILE"
  fi
  write_state "$next_recorder_state"
  echo "$(timestamp) raw_recorder exited status=$status reason=$exit_reason; retrying in ${recorder_retry_seconds}s" >&2
  sleep "$recorder_retry_seconds"
done
