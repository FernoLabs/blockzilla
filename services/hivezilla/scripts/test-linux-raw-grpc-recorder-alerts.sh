...
token_file=$fixture_root/telegram-token
printf '%s\n' '123456789:abcdefghijklmnopqrstuvwxyzABCDEFGH' > "$token_file"
export BLOCKZILLA_TELEGRAM_BOT_TOKEN_FILE=$token_file
...
(
  ...
  mkdir -p "$OUTPUT_DIR"
  printf '%s\n' '{"schema_version":1,"endpoint":"test","cluster_id":"solana-mainnet","origin_node_id":"source-node-01","source_id":"grpc-raw-source-backup","journal_id":[17,17,17,17,17,17,17,17,17,17,17,17,17,17,17,17],"replication_journal_id":null,"replication_sequence_base":null,"payload_format_version":2}' > "$IDENTITY_FILE"
  printf '%s\n' '{"schema_version":1,"cluster_id":"solana-mainnet","origin_node_id":"source-node-01","source_id":"grpc-raw-source-backup","journal_id":"11111111111111111111111111111111","through_sequence":0,"primary_term":1,"updated_unix_secs":1}' > "$receiver_ack_file"
  ...
  grep -q '^Data: source host keeps all unconfirmed blocks\. Indexing is a separate check\.$' \
  ...
  printf '%s\n' '{"schema_version":1,"cluster_id":"solana-mainnet","origin_node_id":"source-node-01","source_id":"grpc-raw-source-backup","journal_id":"11111111111111111111111111111111","through_sequence":1,"primary_term":1,"updated_unix_secs":2}' > "$receiver_ack_file"
  monitor_primary_sync_alert
...
grep -q 'Backblaze upload finished, but source host space did not increase' \
  "$(alert_file generation_backlog active)"
...
# why source host storage remains pinned.
...
grep -q '^Status: Blockzilla has not confirmed these blocks\. source host cleanup is paused; saved data is safe\.$' \
  "$blocked_ack_message"
grep -q '^Storage: <1 MB free\. 4 backup batches kept locally until Blockzilla confirms them\.$' \
  "$blocked_ack_message"
grep -q '^Action: Nothing was deleted\. Restore Blockzilla sync or add source host disk space\. Cloudflare R2 uploads continue\.$' \
  "$blocked_ack_message"
...
grep -q '^Status: Cloudflare R2 upload failed\. source host still has room for recording; saved data is safe\.$' \
  "$r2_spill_message"
...
grep -q 'Cannot read source host disk size' \
  "$(alert_file generation_backlog active)"
grep -q ' ERROR$' "$(alert_file generation_backlog delivered)"
discard_alert generation_backlog
(
...
test ! -s "$failed_gate_upload_log"
grep -q 'Cannot read source host free space' \
  "$(alert_file generation_backlog active)"
discard_alert generation_backlog
(
  validate_data_volume() { return 0; }
...
grep -q '3 backup batches are kept locally' \
  "$(alert_file generation_backlog active)"
grep -q '^Action: Nothing is deleted without Blockzilla confirmation\. Add source host disk space and restore Blockzilla sync\.$' \
  "$(alert_file generation_backlog active)"
...
progress_report=$fixture_root/raw-progress-report.json
cat > "$progress_report" <<'JSON'
{
  "frames_written": 7,
  "outcome": "retryable",
...
JSON
test "$(raw_child_report_backoff_seconds "$progress_report")" \
  -eq "$PROGRESS_RESTART_DELAY_SECS"
PROGRESS_RESTART_DELAY_SECS=2
test "$(raw_child_report_backoff_seconds "$progress_report")" -eq 2
PROGRESS_RESTART_DELAY_SECS=0

# Auth/credit rejection keeps its long backoff even if an established stream
# had already committed frames before access was revoked.
progress_auth_report=$fixture_root/raw-progress-auth-report.json
cat > "$progress_auth_report" <<'JSON'
{
  "frames_written": 7,
  "outcome": "retryable",
  "retry_reason": "unauthenticated",
  "action_required": true,
  "replay_unavailable": false
}
JSON
test "$(raw_child_report_backoff_seconds "$progress_auth_report")" \
  -eq "$UPSTREAM_BLOCKED_BACKOFF_SECS"
