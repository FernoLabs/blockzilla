...
SOURCE_SERVER_CERTIFICATE=${BLOCKZILLA_PULL_SOURCE_CERTIFICATE_FILE:-/run/secrets/source_pull_source_certificate}
SOURCE_SERVER_PRIVATE_KEY=${BLOCKZILLA_PULL_SOURCE_PRIVATE_KEY_SOURCE:-/run/secrets/source_pull_source_private_key}
...
SOURCE_DATA_READ_ONLY=${BLOCKZILLA_PULL_SOURCE_RAW_DATA_READ_ONLY:-true}
RUNTIME_PRIVATE_DIRECTORY=${BLOCKZILLA_PULL_SOURCE_RUNTIME_PRIVATE_DIRECTORY:-/tmp/blockzilla-pull-source}
...
case "$SOURCE_DATA_READ_ONLY" in
  true)
    # `test -w` checks mode bits and can report writable even for a Docker
    # read-only volume mount. Require the actual mount and every nested mount
    # to carry `ro` while ACK-GC is disabled.
    awk '
      $5 == "/source-data" {
        source_seen = 1
        if ($6 ~ /(^|,)ro(,|$)/) source_ro = 1
        else source_non_ro = 1
      }
      index($5, "/source-data/") == 1 && $6 !~ /(^|,)ro(,|$)/ {
        nested_rw = 1
      }
      END { exit !(source_seen && source_ro && !source_non_ro && !nested_rw) }
    ' /proc/self/mountinfo || die "source cache mount is not read-only"
    ;;
  false)
    # The Rust source deletes only whole sealed generations covered by its
    # fsynced signed-ACK WAL. Refuse this mode unless the real volume and every
    # nested mount are writable; a partial read-only subtree would stall GC.
    awk '
      $5 == "/source-data" {
        source_seen = 1
        if ($6 ~ /(^|,)ro(,|$)/) source_ro = 1
      ...
      END { exit !(source_seen && source_rw && !source_ro && !nested_ro) }
    ' /proc/self/mountinfo || die "source cache mount is not read-write"
    [ -w /source-data/grpc-cache ] || die "source cache directory is not writable"
    ;;
  *)
    die "source data read-only policy is invalid"
    ;;
esac
require_regular_readable_file \
...
exec "$HIVEZILLA_BIN" serve-grpc-raw-pull-source \
  --config "$CONFIG_FILE"
