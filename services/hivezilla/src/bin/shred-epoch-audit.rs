...
use hivezilla::ingest::{
    ...
    decode_stored_shred, deshred_complete_data_slot, ensure_parseable_shred_header_matches,
...
impl SlotExtent {
    fn new(sequence: u64, datagram_bytes: usize, full_shred_parse_failed: bool) -> Result<Self> {
        let datagram_bytes =
        ...
        Ok(Self {
            ...
            record_count: 1,
            invalid_stored_shred_records: if full_shred_parse_failed { 1 } else { 0 },
    ...
    fn observe(
        ...
        self.invalid_stored_shred_records = self
            .invalid_stored_shred_records
            .checked_add(if full_shred_parse_failed { 1 } else { 0 })
            .context("slot invalid-stored-shred count overflow")?;
...
impl SlotBuffer {
    fn new(extent: &SlotExtent) -> Result<Self> {
        ...
        Ok(Self {
            datagrams,
            invalid_stored_shred_records: 0,
            decoded_datagram_bytes: 0,
            resident_bytes,
        })
    }

    fn push(&mut self, datagram: Vec<u8>, full_shred_parse_failed: bool) -> Result<()> {
        ...
        self.invalid_stored_shred_records = self
            .invalid_stored_shred_records
            .checked_add(if full_shred_parse_failed { 1 } else { 0 })
            .context("buffered invalid-stored-shred count overflow")?;
...
fn main() -> Result<()> {
    ...
    let pass_one = scan_frozen_prefix(&args, &identity, after, "pass1", |record| {
        let Some(decoded) = decode_epoch_record(record, coverage_start, coverage_end)? else {
            return Ok(());
        };
        let DecodedEpochRecord {
            slot,
            sequence,
            datagram,
            full_shred_parse_failed,
        } = decoded;
        match extents.entry(slot) {
            Entry::Vacant(entry) => {
                entry.insert(SlotExtent::new(
                    sequence,
                    datagram.len(),
                    full_shred_parse_failed,
                )?);
            ...
            Entry::Occupied(mut entry) => {
                entry
                    .get_mut()
                    .observe(sequence, datagram.len(), full_shred_parse_failed)?
            }
        }
        Ok(())
    })?;
    ...
    let pass_two = scan_frozen_prefix(&args, &identity, after, "pass2", |record| {
        ...
        buffer.push(datagram, full_shred_parse_failed)?;
        resident_bytes = resident_bytes
            .checked_add(
        ...
        let buffer = buffers.remove(&slot).expect("completed slot buffer exists");
            ...
            buffer.invalid_stored_shred_records == extent.invalid_stored_shred_records,
            "slot {slot} invalid-stored-shred count differs between passes"
        );
        resident_bytes = resident_bytes
...
fn decode_epoch_record(
    ...
    coverage_start: u64,
    coverage_end: u64,
) -> Result<Option<DecodedEpochRecord>> {
    ...
    let datagram = decode_stored_shred(record.metadata.payload_format_version, &record.payload)
        .context("decode versioned raw shred from epoch journal")?;
    if slot < coverage_start || slot > coverage_end {
        return Ok(None);
    }
    ensure_parseable_shred_header_matches(&datagram, slot, kind, shred_index, fec_set_index)?;
    let full_shred_parse_failed = match Shred::new_from_serialized_shred(datagram.clone()) {
        Ok(shred) => {
    ...
    Ok(Some(DecodedEpochRecord {
        slot,
        sequence: record.metadata.observation.sequence,
        datagram,
        full_shred_parse_failed,
    }))
}
...
mod tests {
    use super::*;
    use hivezilla::ingest::{
        IngressRecordMeta, ObservationId, RAW_SOLANA_SHRED_V1, ZSTD_SOLANA_SHRED_V1,
    };

    fn malformed_epoch_record(format: u16, slot: u64) -> SpoolRecord {
        let mut datagram = vec![0u8; 83];
        datagram[64] = 0x90;
        datagram[65..73].copy_from_slice(&slot.to_le_bytes());
        datagram[73..77].copy_from_slice(&0u32.to_le_bytes());
        datagram[77..79].copy_from_slice(&50_093u16.to_le_bytes());
        datagram[79..83].copy_from_slice(&0u32.to_le_bytes());
        let payload = match format {
            RAW_SOLANA_SHRED_V1 => datagram,
            ZSTD_SOLANA_SHRED_V1 => {
                zstd::bulk::compress(&datagram, 1).expect("compress malformed test shred")
            }
            _ => b"unknown".to_vec(),
        };
    ...
    #[test]
    fn epoch_decoder_keeps_v3_and_v4_parse_failures_as_incomplete_evidence() {
        for format in [RAW_SOLANA_SHRED_V1, ZSTD_SOLANA_SHRED_V1] {
            let decoded = decode_epoch_record(malformed_epoch_record(format, 42), 42, 42)
                .expect("decode versioned record")
                .expect("record is in coverage");
            assert_eq!(decoded.slot, 42);
            assert!(decoded.full_shred_parse_failed);
            assert_eq!(decoded.datagram.len(), 83);
        }
    }

    #[test]
    fn epoch_decoder_hard_fails_unknown_payload_formats() {
        let error = decode_epoch_record(malformed_epoch_record(99, 42), 42, 42).unwrap_err();
        assert!(format!("{error:#}").contains("unsupported stored shred payload format"));
    }

    #[test]
    fn epoch_decoder_hard_fails_parseable_header_metadata_mismatches() {
        let mut record = malformed_epoch_record(RAW_SOLANA_SHRED_V1, 42);
        record.metadata.logical_key = LogicalKey::Shred {
            slot: 43,
            kind: ShredKind::Data,
            shred_index: 0,
            fec_set_index: Some(0),
        };
        let error = decode_epoch_record(record, 43, 43).unwrap_err();
        assert!(format!("{error:#}").contains("decoded common header"));
    }

    #[test]
    fn malformed_full_shred_is_reported_as_incomplete_slot_evidence() {
        let decoded = decode_epoch_record(malformed_epoch_record(RAW_SOLANA_SHRED_V1, 42), 42, 42)
            .expect("decode legacy raw record")
            .expect("record is in coverage");
        let extent = SlotExtent::new(
            decoded.sequence,
            decoded.datagram.len(),
            decoded.full_shred_parse_failed,
        )
        .expect("build slot extent");

        let outcome = audit_slot(42, extent, vec![decoded.datagram], Vec::new());

        assert_eq!(outcome.extent.invalid_stored_shred_records, 1);
        assert_eq!(
            outcome.reconstruction.failure_category,
            Some("invalid_stored_shred")
        );
    }
