    ...
    fn push(
        ...
        if require_initial_sequence_zero {
            validate_raw_replication_batch(&records, &stream, self.config.receiver_limits, true)
                .map_err(ReceiverManagerError::InitialValidation)?;
        }
...
enum ReceiverManagerError {
    ...
    Open(anyhow::Error),
    InitialValidation(anyhow::Error),
    Push(anyhow::Error),
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ReceiverManagerErrorKind {
    OpenLimit,
    KnownLimit,
    StatePoisoned,
    UnsafeExistingStream,
    ...
    InitialDigest,
    InitialProtobuf,
    InitialBlockAccounts,
    InitialBlockTransactions,
    InitialBlockEntries,
    InitialBlockOther,
    InitialOther,
    PushCapacity,
...
fn manager_error_kind(error: &ReceiverManagerError) -> ReceiverManagerErrorKind {
    match error {
        ReceiverManagerError::OpenLimit => ReceiverManagerErrorKind::OpenLimit,
        ReceiverManagerError::KnownLimit => ReceiverManagerErrorKind::KnownLimit,
        ReceiverManagerError::StatePoisoned => ReceiverManagerErrorKind::StatePoisoned,
        ReceiverManagerError::UnsafeExistingStream => {
            ReceiverManagerErrorKind::UnsafeExistingStream
        }
        ...
        ReceiverManagerError::Push(error) if is_input_error(error) => {
            ReceiverManagerErrorKind::PushInput
        }
...
fn initial_validation_error_kind(error: &anyhow::Error) -> ReceiverManagerErrorKind {
    for cause in error.chain() {
        let message = cause.to_string();
        if message.contains("decompress") {
            return ReceiverManagerErrorKind::InitialCompression;
        }
        if message.contains("SHA-256") || message.contains("content digest") {
            return ReceiverManagerErrorKind::InitialDigest;
        }
        if message.contains("stream identity")
            || message.contains("replication offer belongs")
            || message.contains("protocol version")
            || message.contains("payload format")
            || message.contains("commitment")
        {
            return ReceiverManagerErrorKind::InitialIdentity;
        }
        if message.contains("sequence") {
            return ReceiverManagerErrorKind::InitialSequence;
        }
        if message.contains("protobuf") {
            return ReceiverManagerErrorKind::InitialProtobuf;
        }
        if message.contains("updated account") || message.contains("account payload") {
            return ReceiverManagerErrorKind::InitialBlockAccounts;
        }
        if message.contains("transaction") {
            return ReceiverManagerErrorKind::InitialBlockTransactions;
        }
        if message.contains("entry") || message.contains("PoH") {
            return ReceiverManagerErrorKind::InitialBlockEntries;
        }
        if message.contains("block") {
            return ReceiverManagerErrorKind::InitialBlockOther;
        }
        if message.contains("batch")
            || message.contains("compressed")
            || message.contains("uncompressed")
        ...
        {
            return ReceiverManagerErrorKind::InitialLimit;
        }
    }
    ReceiverManagerErrorKind::InitialOther
}

fn manager_error_status(error: ReceiverManagerError) -> Status {
    // Never log the underlying error: filesystem paths, payload-derived validation details, or
    // deployment identifiers may be present in its context chain. This closed enum is sufficient
    // for an operator to distinguish capacity, input, state, and durable-open failures.
    tracing::warn!(
        error_kind = ?manager_error_kind(&error),
        "durable replication receiver rejected a request"
    );
    match error {
        ...
        ReceiverManagerError::KnownLimit => {
            ...
            // filling Source node while retrying forever.
            Status::failed_precondition("replication known-stream limit reached")
        }
        ReceiverManagerError::InitialValidation(_) => {
...
mod tests {
        ...
        format!(
            r#"{{
                ...
                "clients": [{{
                    ...
                    "identities": [{{
                        "cluster_id": "mainnet",
                        "origin_node_id": "source-node-relay",
    ...
    fn certificate_is_bound_to_exact_cluster_origin_and_source() {
        ...
        assert!(authenticated.permits(&stream("source-node-relay", "yellowstone-blocks")));
        let mut wrong_cluster = stream("source-node-relay", "yellowstone-blocks");
        ...
        assert!(!authenticated.permits(&stream("source-node-relay", "other-source")));
        assert!(matches!(
    ...
    fn invalid_new_batch_does_not_consume_a_journal_or_create_the_root() {
        ...
        assert!(matches!(
            ...
            Err(ReceiverManagerError::InitialValidation(_))
        ));
        assert!(!root.exists());
    ...
    fn mapped_errors_do_not_disclose_underlying_paths_or_payloads() {
        ...
        assert_eq!(
            manager_error_kind(&error),
            ReceiverManagerErrorKind::InitialBlockTransactions
        );
