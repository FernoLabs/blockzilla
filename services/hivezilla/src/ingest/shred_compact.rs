...
use super::{
    ...
    shred_udp::{decode_stored_shred, ensure_parseable_shred_header_matches},
};
...
pub fn trial_deshred_spool(config: ShredSpoolTrialConfig) -> Result<ShredSpoolTrialReport> {
    ...
    let snapshot = read_spool_committed_snapshot_after(
        ...
        |record| {
            ...
            ensure_parseable_shred_header_matches(
                &datagram,
                slot,
                kind,
                shred_index,
...
mod tests {
    use super::*;
    use crate::ingest::{
        IngressRecordMeta, ObservationId, RAW_SOLANA_SHRED_V1, SpoolOptions, SpoolWriter,
    ...
    #[test]
    fn trial_reports_malformed_v3_and_v4_records_without_aborting_other_slots() {
        ...
        let mut spool =
            SpoolWriter::open(directory.path(), identity.clone(), options).expect("open journal");
        for (sequence, format) in [RAW_SOLANA_SHRED_V1, ZSTD_SOLANA_SHRED_V1]
            .into_iter()
            .enumerate()
        {
            ...
            let stored = match format {
                ...
                ZSTD_SOLANA_SHRED_V1 => zstd::bulk::compress(&datagram, 1).unwrap(),
                _ => unreachable!(),
            };
        ...
        let report = trial_deshred_spool(ShredSpoolTrialConfig {
            spool_root: directory.path().to_path_buf(),
            identity,
            ...
            min_slot: None,
            max_failure_samples: 2,
        })
        .expect("trial must preserve malformed evidence");

        assert_eq!(report.scanned_records, 2);
        assert_eq!(report.invalid_stored_shred_records, 2);
        assert_eq!(report.failures.invalid_stored_shred_slots, 2);
        assert_eq!(report.failure_samples.len(), 2);
        assert!(
            report
                .failure_samples
                .iter()
                .all(|sample| sample.category == "invalid_stored_shred")
        );
    }
    ...
    fn classifies_fec_identity_and_geometry_conflicts_separately() {
        let invalid = anyhow::anyhow!("slot contains 2 invalid stored shred records");
        ...
        assert_eq!(
            shred_reconstruction_failure_category(&invalid),
            "invalid_stored_shred"
        );
    }
