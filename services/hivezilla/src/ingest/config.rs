...
/// Public-safe source labels are bounded so recorder status cannot become an unbounded peer list.
...
pub enum SourceInputConfig {
    ...
    ShredUdp {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        auth: Option<SourceAuthConfig>,
        /// Optional names for exact UDP senders. Names are used only for post-fsync status
        /// attribution; unnamed senders remain accepted and anonymous.
...
mod tests {
    ...
    fn valid_replica_json(mode: &str) -> Value {
        ...
        value["role"] = json!({
            ...
            "upstream": {
                "endpoint": "https://192.0.2.45:9555",
