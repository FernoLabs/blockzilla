...
/// Transport-level liveness for the long-lived Sync RPC. HTTP/2 PING detects a dead h2 peer in
/// roughly forty seconds. TCP keepalive is a second, kernel-level fence for broken paths; it is
/// installed directly on accepted sockets because Tonic intentionally ignores its TCP keepalive
/// builder options when `serve_with_incoming` is used.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct PullSourceTransportPolicy {
...
impl PullSourceTransportPolicy {
    ...
    fn validate(self) -> Result<Self> {
            ...
            !self.http2_keepalive_interval.is_zero()
                && !self.http2_keepalive_timeout.is_zero()
                && self.http2_keepalive_timeout < self.http2_keepalive_interval
                && !self.tcp_keepalive_idle.is_zero()
                && !self.tcp_keepalive_interval.is_zero()
                && self.tcp_keepalive_retries > 0,
            "pull-source transport keepalive policy is invalid"
        );
        for duration in [self.tcp_keepalive_idle, self.tcp_keepalive_interval] {
            ensure!(
                duration.subsec_nanos() == 0 && duration.as_secs() <= libc::c_int::MAX as u64,
                "pull-source TCP keepalive duration does not fit the socket API"
            );
        }
        ensure!(
            self.tcp_keepalive_retries <= libc::c_int::MAX as u32,
            "pull-source TCP keepalive retry count does not fit the socket API"
        );
        Ok(self)
    }
}

/// Strict executable configuration for the standalone Deployment pull-source service.
...
pub struct ShredSpoolGcConfig {
    /// Retention is deliberately disabled when this field or the whole object is omitted.
    pub enabled: bool,
    #[serde(default)]
    pub control_uid: Option<u32>,
    #[serde(default)]
...
/// client. ACK-driven deletion is a separate, default-off rollout.
#[derive(Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
#[cfg(unix)]
fn configure_pull_source_tcp_keepalive<S: AsRawFd>(
    stream: &S,
    policy: PullSourceTransportPolicy,
) -> io::Result<()> {
    let descriptor = stream.as_raw_fd();
    set_socket_i32(descriptor, libc::SOL_SOCKET, libc::SO_KEEPALIVE, 1)?;
    let idle = duration_socket_seconds(policy.tcp_keepalive_idle)?;
    let interval = duration_socket_seconds(policy.tcp_keepalive_interval)?;
    let retries = libc::c_int::try_from(policy.tcp_keepalive_retries)
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, "TCP keepalive retry overflow"))?;

    #[cfg(any(target_os = "linux", target_os = "android"))]
    set_socket_i32(descriptor, libc::IPPROTO_TCP, libc::TCP_KEEPIDLE, idle)?;
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    set_socket_i32(descriptor, libc::IPPROTO_TCP, libc::TCP_KEEPALIVE, idle)?;
    #[cfg(any(
        target_os = "linux",
        target_os = "android",
        target_os = "macos",
    ...
    {
        set_socket_i32(descriptor, libc::IPPROTO_TCP, libc::TCP_KEEPINTVL, interval)?;
        set_socket_i32(descriptor, libc::IPPROTO_TCP, libc::TCP_KEEPCNT, retries)?;
    }
    Ok(())
}

#[cfg(not(unix))]
fn configure_pull_source_tcp_keepalive(
    _stream: &tokio::net::TcpStream,
    _policy: PullSourceTransportPolicy,
) -> io::Result<()> {
    Ok(())
}

#[cfg(unix)]
fn duration_socket_seconds(duration: Duration) -> io::Result<libc::c_int> {
    if duration.is_zero() || duration.subsec_nanos() != 0 {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "TCP keepalive duration must be positive whole seconds",
        ));
    }
    libc::c_int::try_from(duration.as_secs()).map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "TCP keepalive duration overflow",
        )
    })
}
...
fn set_socket_i32(
    ...
    // SAFETY: `descriptor` remains borrowed from the live accepted socket; `value` points to a
    // correctly sized integer for the duration of this synchronous syscall.
    let result = unsafe {
        libc::setsockopt(
            descriptor,
            level,
...
fn validate_shred_spool_runtime_config(
    ...
    validate_shred_spool_gc_config(config.gc)?;
    for (label, path) in [
        ("server certificate", &config.tls.server_certificate_file),
        ("server private key", &config.tls.server_private_key_file),
        ("client CA", &config.tls.client_ca_file),
...
fn validate_shred_spool_gc_config(gc: ShredSpoolGcConfig) -> Result<()> {
    ensure!(
        (!gc.enabled
            && gc.control_uid.is_none()
            && gc.recorder_gid.is_none()
            && gc.max_retirements_per_process == 0)
            || (gc.enabled
                && gc.control_uid == Some(0)
                && gc.recorder_gid.is_some_and(|gid| gid != 0)
                && gc.max_retirements_per_process > 0),
...
mod tests {
    ...
    #[test]
    fn shred_gc_omission_defaults_off_and_enablement_is_strict() {
        let mut value = serde_json::json!({
            "schema_version": 1,
            "bind": "127.0.0.1:9443",
            "tls": {
                "server_certificate_file": "/control/server.pem",
                "server_private_key_file": "/control/server.key",
                "client_ca_file": "/control/client-ca.pem",
                "handshake_timeout_ms": 10000
            },
            "allowed_nodes_file": "/control/allowed.json",
            "spool_root": "/source-data/shreds",
            "cluster_id": "mainnet",
            "origin_node_id": "source-a",
            "source_id": "shred-udp",
            "journal_id": "01010101010101010101010101010101",
            "recorder_status_file": "/control/recorder.json",
            "cumulative_ack_wal_file": "/control/ack.wal",
            "ack_status_file": "/control/ack.json",
            "expected_primary_id": "primary",
            "trusted_receipt_keys": [{
                "key_id": "key-a",
                "public_key_file": "/control/key.pem"
            }],
            "limits": {
                "max_records": 1,
                "max_batch_compressed_bytes": 1024,
                "max_batch_uncompressed_bytes": 2048,
                "max_compressed_record_bytes": 1024,
                "max_uncompressed_record_bytes": 2048
            }
        });
        let omitted: ShredSpoolPullRuntimeConfig =
            serde_json::from_value(value.clone()).expect("omitted GC config");
        assert_eq!(omitted.gc, ShredSpoolGcConfig::default());
        validate_shred_spool_gc_config(omitted.gc).expect("default-off GC");

        value["gc"] = serde_json::json!({
            "enabled": true,
            "control_uid": 0,
            "recorder_gid": 10001,
            "max_retirements_per_process": 1
        });
        let enabled: ShredSpoolPullRuntimeConfig =
            serde_json::from_value(value.clone()).expect("enabled GC config");
        validate_shred_spool_gc_config(enabled.gc).expect("valid enabled GC");

        value["gc"]["max_retirements_per_process"] = serde_json::json!(0);
        let invalid: ShredSpoolPullRuntimeConfig =
            serde_json::from_value(value).expect("structurally valid GC config");
        assert!(validate_shred_spool_gc_config(invalid.gc).is_err());
    }
    ...
    fn config_debug_never_exposes_paths() {
        ...
        for secret_path in [
            "/run/secrets/source-node_pull_source_certificate",
    ...
    #[test]
    fn production_transport_policy_bounds_http2_and_tcp_half_open_detection() {
        let policy = PullSourceTransportPolicy::production()
            .validate()
            .expect("production keepalive policy");
        assert_eq!(policy.http2_keepalive_interval, Duration::from_secs(30));
        assert_eq!(policy.http2_keepalive_timeout, Duration::from_secs(10));
        assert_eq!(policy.tcp_keepalive_idle, Duration::from_secs(30));
        assert_eq!(policy.tcp_keepalive_interval, Duration::from_secs(10));
        assert_eq!(policy.tcp_keepalive_retries, 3);

        let mut invalid = policy;
        invalid.http2_keepalive_timeout = invalid.http2_keepalive_interval;
        assert!(invalid.validate().is_err());
        invalid = policy;
        invalid.tcp_keepalive_retries = 0;
        assert!(invalid.validate().is_err());
    }

    #[cfg(unix)]
    #[test]
    fn accepted_socket_receives_the_production_tcp_keepalive_policy() {
        let listener = std::net::TcpListener::bind("127.0.0.1:0").expect("bind test listener");
        let address = listener.local_addr().expect("test listener address");
        let client = std::net::TcpStream::connect(address).expect("connect test client");
        let (server, _) = listener.accept().expect("accept test server socket");
        let policy = PullSourceTransportPolicy::production();
        configure_pull_source_tcp_keepalive(&server, policy).expect("configure TCP keepalive");

        assert!(
            get_socket_i32(server.as_raw_fd(), libc::SOL_SOCKET, libc::SO_KEEPALIVE)
                .expect("read SO_KEEPALIVE")
                != 0
        );
        #[cfg(any(target_os = "linux", target_os = "android"))]
        assert_eq!(
            get_socket_i32(server.as_raw_fd(), libc::IPPROTO_TCP, libc::TCP_KEEPIDLE)
                .expect("read TCP_KEEPIDLE"),
            30
        );
        #[cfg(any(target_os = "macos", target_os = "ios"))]
        assert_eq!(
            get_socket_i32(server.as_raw_fd(), libc::IPPROTO_TCP, libc::TCP_KEEPALIVE)
                .expect("read TCP_KEEPALIVE"),
            30
        );
        #[cfg(any(
            target_os = "linux",
        ...
        {
            assert_eq!(
                get_socket_i32(server.as_raw_fd(), libc::IPPROTO_TCP, libc::TCP_KEEPINTVL)
                    .expect("read TCP_KEEPINTVL"),
                10
            );
            assert_eq!(
                get_socket_i32(server.as_raw_fd(), libc::IPPROTO_TCP, libc::TCP_KEEPCNT)
                    .expect("read TCP_KEEPCNT"),
                3
            );
        }
        drop(client);
    ...
    fn get_socket_i32(
        ...
        // SAFETY: the descriptor belongs to a live test socket and both output pointers remain
    ...
    fn post_commit_status_is_atomic_fsynced_and_monitor_compatible() {
        ...
        let ack = CumulativePrimaryAck {
            ...
            stream: ReplicationStreamId {
                ...
                origin_node_id: "source-node-relay".into(),
