...
impl From<wincode::error::Error> for RepairWireError {
    fn from(error: wincode::error::Error) -> Self {
        Self::Codec(error)
    }
}
...
pub fn parse_verified_ping(packet: &[u8]) -> Option<RepairPing> {
    ...
    let RepairResponse::Ping(ping) = wincode::deserialize(packet).ok()?;
...
mod tests {
    ...
    fn window_index_matches_agave_v4_1_2_golden() {
        ...
        let protocol: RepairProtocol = wincode::deserialize(&packet).unwrap();
    ...
    fn verified_ping_and_pong_match_agave_v4_1_2_goldens() {
        ...
        let ping_packet = wincode::serialize(&RepairResponse::Ping(ping)).unwrap();
        ...
        let RepairProtocol::Pong(pong) = wincode::deserialize(&pong_packet).unwrap() else {
    ...
    fn malformed_or_forged_ping_is_not_accepted() {
        ...
        let server = Keypair::new_from_array([11; 32]);
        let ping = RepairPing::new([22; REPAIR_PING_TOKEN_BYTES], &server);
        let mut packet = wincode::serialize(&RepairResponse::Ping(ping)).unwrap();
