...
impl<T: RepairTrust> RepairRuntime<T> {
    ...
    fn refresh_repair_wal_stats(&mut self) {
        ...
        self.stats.repair_wal_filesystem_available_bytes =
            Some(snapshot.filesystem_available_bytes);
...
mod tests {
    ...
    async fn answers_only_a_ping_signed_by_the_address_bound_peer() {
        ...
        let ping =
            wincode::serialize(&TestRepairResponse::Ping(Ping::new([7; 32], &peer_key))).unwrap();
        peer_socket
