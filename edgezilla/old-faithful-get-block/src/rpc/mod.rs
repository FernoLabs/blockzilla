...
pub async fn handle_rpc(mut req: Request, env: &Env) -> Result<Response> {
    ...
    match method {
        "getBlock" => bytes_result(id, get_block_json(env, params.as_ref()).await),
        "getBlockTime" => bytes_result(id, get_block_time(env, params.as_ref()).await),
...
async fn get_block_json(
    ...
    let block = fetch_block(env, slot, config.rewards, true).await?;
...
async fn get_block_time(
    ...
    let block = fetch_block(env, slot, false, false).await?;
    Ok(
...
async fn fetch_block(
    ...
    let block = fetch_block_from_archive(env, slot, include_rewards, require_previous_blockhash)
        .await
