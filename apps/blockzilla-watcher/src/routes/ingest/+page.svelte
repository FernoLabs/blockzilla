<script lang="ts">
  ...
  import ProgressTrend, { type TrendPoint } from '$lib/ProgressTrend.svelte';
  import {
  ...
  const rawShred = $derived(watcher.rawShred);
  const rawShredFresh = $derived(
    rawShred !== null &&
      rawShred.hivezilla.status_fresh &&
      rawShred.updated_unix_secs <= nowUnixSecs + 5 &&
      nowUnixSecs <= rawShred.updated_unix_secs + 30
  );
  const fresh = $derived(
    status !== null &&
  ...
  const snapshot = $derived(watcherSnapshotFresh ? watcher.snapshot : null);
  const activeProcesses = $derived.by<ProcessRow[]>(() => {
    const processes = watcher.runtimeOperations?.process_io.processes ?? watcher.snapshot?.process_io?.processes ?? [];
    const measured = processes
      .filter((process) => process.blockzilla_owned !== false)
      .map((process) => ({
        id: process.id,
        name: process.name,
        pid: process.pid,
        ...
        read_mib_per_sec: process.read_mib_per_sec ?? null,
        write_mib_per_sec: process.write_mib_per_sec ?? null
      }));
    const laneFallback = (watcher.snapshot?.lanes ?? [])
      .filter((lane) => lane.pid !== null && !measured.some((process) => process.pid === lane.pid))
      .map((lane) => ({
        id: `lane:${lane.id}`,
        name: `${lane.kind} · epoch ${formatInteger(lane.epoch)}`,
        pid: lane.pid!,
        cpu_percent: null,
        rss_bytes: lane.rss_bytes ?? lane.progress.rss_bytes,
        read_mib_per_sec: lane.progress.disk_read_mib_per_sec ?? null,
        write_mib_per_sec: lane.progress.disk_write_mib_per_sec ?? null
      }));
    return [...measured, ...laneFallback]
      .toSorted((left, right) =>
        (right.rss_bytes ?? 0) - (left.rss_bytes ?? 0) ||
        (right.cpu_percent ?? 0) - (left.cpu_percent ?? 0)
      )
      .slice(0, 8);
  });
  const rawSlotPoints = $derived(trendPoints(watcher.rawShredTrend, 'latestSlot'));
  const rawAcceptedPoints = $derived(trendPoints(watcher.rawShredTrend, 'acceptedTotal'));
  const archiveBlockPoints = $derived(trendPoints(watcher.watcherTrend, 'archiveBlocksDone'));
  const rawSlotRate = $derived(trendRate(rawSlotPoints));
  const rawAcceptedRate = $derived(trendRate(rawAcceptedPoints));
  const archiveBlockRate = $derived(trendRate(archiveBlockPoints));
  const stages = $derived.by<Stage[]>(() => {
    if (!status) return [];
    const rows: Stage[] = [
      {
        id: 'raw-shreds',
        name: 'Raw shred recorder',
        role: 'Lossless Hetzner spool before block reconstruction',
        tone: rawShredTone(),
        state: rawShredStateLabel(),
        primary: rawShred
          ? throughSlotLabel('Durable through', rawShred.hivezilla.latest_slot)
          : 'Raw-shred telemetry not connected',
        secondary: rawShred
          ? `${formatStorageBytes(rawShred.hivezilla.spool_bytes)} of ${formatStorageBytes(rawShred.hivezilla.spool_max_bytes)} spool · ${formatStorageBytes(rawShred.hivezilla.filesystem_free_bytes)} free`
          : 'The NAS pull consumer is not inferred from this status',
        updatedUnixSecs: rawShred?.updated_unix_secs ?? null
      },
      {
        id: 'upstream',
        name: 'Triton ingest',
  ...
  function trendPoints<T extends { at: number }, K extends keyof T>(samples: T[], key: K): TrendPoint[] {
    return samples.flatMap((sample) =>
      typeof sample[key] === 'number' ? [{ at: sample.at, value: sample[key] as number }] : []
    );
  }

  function trendRate(points: TrendPoint[]) {
  ...
  function trendDelta(points: TrendPoint[]) {
    return points.length < 2 ? 0 : points.at(-1)!.value - points[0].value;
  }

  function trendWindow(points: TrendPoint[]) {
    if (points.length < 2) return 'this page session';
    const seconds = points.at(-1)!.at - points[0].at;
  ...
  function rawShredTone(): Tone {
    if (!rawShred || !rawShredFresh || rawShred.hivezilla.availability !== 'available') return 'unknown';
    if (rawShred.hivezilla.state === 'receiving') return 'healthy';
    if (rawShred.hivezilla.state === 'waiting') return 'warning';
    if (rawShred.hivezilla.state === 'stalled' || rawShred.hivezilla.state === 'stopped') return 'error';
    return 'unknown';
  }

  function rawShredStateLabel() {
    if (!rawShred) return 'not connected';
    if (!rawShredFresh) return 'status stale';
    const labels: Record<string, string> = {
      receiving: 'recording',
      waiting: 'waiting for shreds',
      stalled: 'stalled',
      stopped: 'stopped',
      unavailable: 'unavailable'
    };
    return labels[rawShred.hivezilla.state] ?? rawShred.hivezilla.state;
  }
...
<main>
  <div class="page-heading">
    <div>
      ...
      <p>Raw shred retention, block ingest, and durable receiver acknowledgement.</p>
    </div>
  ...
  <section class="telemetry" aria-labelledby="progress-title">
    <div class="section-heading">
      <h3 id="progress-title">Progress over time</h3>
      <span>Last {trendWindow(rawSlotPoints.length > 1 ? rawSlotPoints : archiveBlockPoints)} · resets when this page reloads</span>
    </div>
    {#if rawSlotPoints.length > 0}
      <ProgressTrend
        title="Raw shred slot"
        points={rawSlotPoints}
        valueLabel={`Slot ${formatInteger(rawSlotPoints.at(-1)?.value)}`}
        rateLabel={`+${formatInteger(trendDelta(rawSlotPoints))} slots · ${formatDecimal(rawSlotRate, 2)} slots/s`}
      />
      <ProgressTrend
        title="Accepted shreds"
        points={rawAcceptedPoints}
        valueLabel={formatInteger(rawAcceptedPoints.at(-1)?.value)}
        rateLabel={`+${formatInteger(trendDelta(rawAcceptedPoints))} · ${formatDecimal(rawAcceptedRate, 1)} shreds/s`}
      />
    {/if}
    {#if archiveBlockPoints.length > 0}
      <ProgressTrend
        title="Archive processing"
        points={archiveBlockPoints}
        valueLabel={`${formatInteger(archiveBlockPoints.at(-1)?.value)} blocks processed`}
        rateLabel={`+${formatInteger(trendDelta(archiveBlockPoints))} · ${formatDecimal(archiveBlockRate, 1)} blocks/s`}
      />
    {/if}
    {#if rawSlotPoints.length === 0 && archiveBlockPoints.length === 0}
      <p class="empty telemetry-empty">Waiting for the first two status samples.</p>
    {/if}
  </section>

  <section class="resources" aria-labelledby="resources-title">
    <div class="section-heading">
      <h3 id="resources-title">Load and processes</h3>
      <span>{snapshot ? 'NAS status updated now' : 'Waiting for a fresh NAS sample'}</span>
    </div>
    {#if snapshot}
      <dl class="resource-strip">
        <div><dt>Load (1m)</dt><dd>{formatDecimal(snapshot.machine.load_1m, 2)}</dd></div>
        <div><dt>Memory available</dt><dd>{formatStorageBytes(snapshot.machine.memory_available_bytes)} / {formatStorageBytes(snapshot.machine.memory_total_bytes)}</dd></div>
        <div><dt>Swap used</dt><dd>{formatStorageBytes(snapshot.machine.swap_used_bytes)} / {formatStorageBytes(snapshot.machine.swap_total_bytes)}</dd></div>
        <div><dt>NAS disk</dt><dd>{formatStorageBytes(snapshot.machine.disk_available_bytes)} free</dd></div>
        <div><dt>Disk I/O</dt><dd>R {formatDecimal(snapshot.machine.archive_device_read_mib_per_sec)} · W {formatDecimal(snapshot.machine.archive_device_write_mib_per_sec)} MiB/s</dd></div>
        <div><dt>Pressure (10s)</dt><dd>memory {formatDecimal(snapshot.machine.memory_pressure_full_avg10, 2)}% · I/O {formatDecimal(snapshot.machine.io_pressure_full_avg10, 2)}%</dd></div>
      </dl>
      <div class="process-table-wrap">
        <table>
          <thead><tr><th>Process</th><th>PID</th><th>CPU</th><th>Memory</th><th>I/O (MiB/s)</th></tr></thead>
          <tbody>
            {#each activeProcesses as process (process.id)}
              <tr>
                <td>{process.name}</td>
                <td>{formatInteger(process.pid)}</td>
                <td>{process.cpu_percent === null || process.cpu_percent === undefined ? '—' : `${formatDecimal(process.cpu_percent, 1)}%`}</td>
                <td>{process.rss_bytes === null || process.rss_bytes === undefined ? '—' : formatStorageBytes(process.rss_bytes)}</td>
                <td>R {formatDecimal(process.read_mib_per_sec)} · W {formatDecimal(process.write_mib_per_sec)}</td>
              </tr>
            {:else}
              <tr><td colspan="5" class="empty">Process telemetry is collecting or inaccessible.</td></tr>
            {/each}
          </tbody>
        </table>
      </div>
    {:else}
      <p class="empty telemetry-empty">The archive snapshot is stale or unavailable.</p>
    {/if}
  </section>
    ...
    <div class="lower-sections">
      <section aria-labelledby="retention-title">
        ...
        <dl class="facts">
          ...
          {#if rawShred}
            <div>
              <dt>Raw spool safety floor</dt>
              <dd>
                {formatStorageBytes(rawShred.hivezilla.filesystem_free_bytes)} free · recorder stops at {formatStorageBytes(rawShred.hivezilla.reserve_free_bytes)}
              </dd>
            </div>
          {/if}
          <div>
            <dt>NAS pull replication</dt>
            <dd>ACK-only retention is enabled when a NAS pull client connects; this page never treats a missing ACK as permission to delete.</dd>
          </div>
          <div>
...
<style>
  ...
  .resource-strip > div:nth-child(3n) {
    border-right: 0;
  }

  .resource-strip > div:nth-last-child(-n + 3) {
    border-bottom: 0;
  }
  ...
  @media (max-width: 820px) {
    ...
    .resource-strip > div:nth-child(2n) {
      border-right: 0;
    }

    .resource-strip > div:nth-last-child(-n + 2) {
      border-bottom: 0;
    }
  }
