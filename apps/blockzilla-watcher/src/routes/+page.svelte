<script lang="ts">
  ...
  const blockTimeGapIndex = $derived(watcher.blockTimeGapIndex);
  ...
  const yearCalendars = $derived(
    buildEpochYearCalendars(
      epochYearStatuses,
      epochCalendar,
      snapshot?.now_unix_secs ?? 0,
      blockTimeGapIndex
    )
  );
  const interruptionDayCount = $derived(
    yearCalendars.reduce((total, year) => total + year.interruption_day_count, 0)
  );
  const missingInterruptionCoverageDayCount = $derived(
    yearCalendars.reduce(
      (total, year) => total + year.missing_interruption_coverage_day_count,
      0
    )
  );
          ...
          {#if archiveProgress.state === 'ready'}
            <div class="archive-progress-track">
              <progress
                ...
                title="Weighted by the CAR bytes to process, including partial progress from active archive compactions."
                aria-label="Overall archive progress by bytes"
                aria-valuetext={`${formatBytes(archiveProgress.bytesDone)} processed of ${formatBytes(archiveProgress.bytesTotal)}; ${formatDecimal(archiveProgress.percent)} percent`}
              >{archiveProgress.percent}%</progress>
              <span>{formatBytes(archiveProgress.bytesDone)} processed of {formatBytes(archiveProgress.bytesTotal)}</span>
            </div>
        ...
        <div class="section-heading">
          ...
          <div class="epoch-header-tools">
            ...
            <div class="legend" aria-label="Epoch status legend">
              ...
              {/if}
              {#if calendarView === 'year' && interruptionDayCount > 0}
                <span>
                  <i class="legend-swatch interruption" aria-hidden="true"></i>
                  observed interruption ≥ {formatDuration(blockTimeGapIndex?.minimum_interruption_secs)}
                </span>
              {/if}
              {#if calendarView === 'year' && missingInterruptionCoverageDayCount > 0}
                <span>
                  <i class="legend-swatch interruption-missing" aria-hidden="true"></i>
                  interruption coverage unavailable
                </span>
              {/if}
            </div>
          </div>
...
<style>
  ...
  .archive-progress-track progress {
    width: 100%;
    height: 8px;
  }
  ...
  .legend-swatch.interruption {
    background: #2d2920;
    box-shadow: inset 0 2px #f1c75b;
  }

  .legend-swatch.interruption-missing {
    border: 1px dashed #7b7b83;
    background: #1b1b1e;
    box-shadow: none;
  }
