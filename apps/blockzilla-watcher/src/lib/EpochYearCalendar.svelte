  ...
  {#each years as year (year.year)}
    <section class="year-calendar" aria-labelledby={`epoch-year-${year.year}`}>
      <div class="year-label">
        ...
        {#if year.interruption_day_count > 0}
          <span>{year.interruption_day_count} interruption {year.interruption_day_count === 1 ? 'day' : 'days'}</span>
        {/if}
        {#if year.missing_interruption_coverage_day_count > 0}
          <span>{year.missing_interruption_coverage_day_count} unindexed {year.missing_interruption_coverage_day_count === 1 ? 'day' : 'days'}</span>
        {/if}
      </div>
      <div class="year-scroll">
        <div class="year-frame" style:--weeks={year.week_count}>
          ...
          <div
            class="day-grid"
            role="img"
            aria-label={`${year.year} daily UTC archive coverage: ${year.archived_epoch_count} of ${year.epoch_count} epochs that started this year are archived; ${year.epoch_count - year.tracked_epoch_count} untracked; ${year.interruption_day_count} days contain observed block-time interruptions; ${year.missing_interruption_coverage_day_count} days have unavailable interruption coverage. Use the Epochs tab for exact archive details.`}
          >
            {#each year.days as day (day.date)}
              {#if day.primary_epoch !== null && !day.future}
                <span
                  ...
                  class:interrupted={day.interruption !== null}
                  class:interruption-coverage-missing={day.interruption_coverage === 'missing'}
                  class:today={day.today}
                  style:--week={day.week + 1}
                  style:--weekday={day.weekday + 1}
                  title={`${day.label}${day.estimated ? ' · estimated chain dates' : ''}${day.interruption_coverage === 'missing' ? ' · interruption index unavailable for one or more epochs' : ''}`}
                  data-date={day.date}
                  aria-hidden="true"
                ></span>
              {:else}
...
<style>
  ...
  .year-day.interrupted {
    box-shadow: inset 0 2px #f1c75b, inset 0 -2px var(--tone-accent);
  }

  .year-day.interrupted.today {
    box-shadow: inset 0 2px #f1c75b, inset 0 -2px var(--tone-accent), 0 0 0 1px #f4f4f5;
  }

  .year-day.interruption-coverage-missing {
    outline: 1px dashed #7b7b83;
    outline-offset: -1px;
  }
