...
const gapIndex = (missingEpochs = []) => ({
  schema_version: 1,
  generated_unix_secs: utc(2026, 7, 17),
  minimum_interruption_secs: 300,
  source_sha256: 'ab'.repeat(32),
  coverage: {
    start_epoch: 7,
    end_epoch: 7,
    expected_epoch_count: 1,
    indexed_epoch_count: missingEpochs.length === 0 ? 1 : 0,
    missing_epochs: missingEpochs,
    indexed_boundary_count: 0,
    source_sidecar_bytes: 200,
    source_gap_rows: 2
  },
  days: [{
    day_start_unix_secs: utc(2026, 7, 15),
    ...
    longest_interruption: {
      ...
      previous_slot: 10,
      next_slot: 11,
      previous_block_time: utc(2026, 7, 15, 10),
      next_block_time: utc(2026, 7, 15, 10) + 900,
      elapsed_secs: 900,
      missing_slots: 0
    }
  }]
...
test('observed interruptions annotate UTC days without replacing archive tone', () => {
  const years = buildEpochYearCalendars([
    status(7, 'complete', 'complete', 1)
  ], [{
    epoch: 7,
    ...
    end_unix_secs: utc(2026, 7, 16),
    precision: 'observed'
  }], utc(2026, 7, 17), gapIndex());

  const day = years[0].days.find((entry) => entry.date === '2026-07-15');
  assert.equal(day?.tone, 'complete');
  assert.equal(day?.interruption?.longest_interruption_secs, 900);
  assert.equal(day?.interruption_coverage, 'covered');
  assert.match(day?.label ?? '', /1 observed block-time interruption · longest 15m/);
  assert.equal(years[0].interruption_day_count, 1);
});

test('missing sidecar coverage remains distinct from a day with no interruption', () => {
  const years = buildEpochYearCalendars([
    status(7, 'partial', 'partial', 6, false)
  ], [{
    epoch: 7,
    start_unix_secs: utc(2026, 7, 14),
    end_unix_secs: utc(2026, 7, 16),
    precision: 'observed'
  }], utc(2026, 7, 17), { ...gapIndex([7]), days: [] });

  const day = years[0].days.find((entry) => entry.date === '2026-07-15');
  assert.equal(day?.interruption, null);
  assert.equal(day?.interruption_coverage, 'missing');
  assert.equal(years[0].missing_interruption_coverage_day_count, 3);
});
