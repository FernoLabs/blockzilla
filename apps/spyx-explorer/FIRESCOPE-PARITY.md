...
- `../../../firescope/apps/web/src/routes/+page.svelte:1040` contains token selection, market summary, pairs, line and OHLC charts, metadata, and trade tables.
- `../../../firescope/apps/web/src/routes/wallet/+page.svelte:95` contains the owner swap search and page controls.
- `../../../firescope/apps/web/src/routes/stats/+page.svelte:72` contains archive, row, file, and source statistics.
...
- `../../crates/blockzilla-spyx-query/src/api.rs` registers health, transaction, owner and other posting, market provenance, summary, pair, trade, and candle routes.
- `../../crates/blockzilla-spyx-query/src/market_builder.rs` applies the fail-closed parser and reconciliation gates. `market_format.rs` defines the source-bound fixed record format. `market_store.rs` validates and serves the published artifact.
- `../../crates/blockzilla-spyx-query/src/store.rs` opens pinned locator and signature handles, validates them in bounded chunks, and uses positioned row reads. `../../crates/blockzilla-spyx-query/src/postings_store.rs:95` opens and verifies the postings artifact with the same fail-closed model.
