...
ORIGIN_NODE_ID=${BLOCKZILLA_RECEIVER_BRIDGE_ORIGIN_NODE_ID:-hetzner-dokploy-01}
SOURCE_ID=${BLOCKZILLA_RECEIVER_BRIDGE_SOURCE_ID:-grpc-raw-hetzner-backup}
...
[ -d /derived ] && [ ! -L /derived ] && [ -w /derived ] || \
  die "/derived must be a writable non-symlink directory"

stop_requested=false
trap 'stop_requested=true' TERM INT HUP
current_error=/tmp/receiver-bridge-current-error
last_error=/tmp/receiver-bridge-last-error

while [ "$stop_requested" = false ]; do
  if /usr/local/bin/blockzilla-live-producer bridge-receiver-grpc-raw \
