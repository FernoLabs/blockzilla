...
# Hetzner source and the NAS-local durable receiver. Stage only its private key
...
RUNTIME_PRIVATE_DIRECTORY=/tmp/blockzilla-pull-client
...
exec /usr/local/bin/blockzilla-live-producer pull-grpc-raw \
  --config "$CONFIG_FILE" \
