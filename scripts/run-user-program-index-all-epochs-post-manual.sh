...
mkdir "$lock" || {
    echo "another User-program index all-epoch publisher holds $lock" >&2
...
if [ -e "$config" ] || [ -L "$config" ]; then
    cmp -s "$config_tmp" "$config" || {
        echo "User-program index batch config changed" >&2
        exit 1
    }
    rm "$config_tmp"
else
...
echo "User-program index all-epoch Post batch complete"
