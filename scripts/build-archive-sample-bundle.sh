...
for format in compact-v2 indexer-v3; do
...
env -u CPATH -u C_INCLUDE_PATH -u CPLUS_INCLUDE_PATH -u LIBRARY_PATH -u SDKROOT \
  ...
  -p blockzilla-read-car -p blockzilla-read-compact-v2 -p blockzilla-read-indexer-v3 "${args[@]}"
mkdir "$bundle/bin"
...
cp crates/blockzilla-example-workloads/src/progress.rs "$bundle/progress.rs"
cp crates/blockzilla-read-sdk/src/query_keys.rs "$bundle/query_keys.rs"
cp crates/blockzilla-read-sdk/src/count_projection.rs "$bundle/count_projection.rs"
cp crates/blockzilla-query-sdk/src/projection_pool.rs "$bundle/projection_pool.rs"
cp -R crates/blockzilla-reader-profile "$bundle/reader-profile-source"
