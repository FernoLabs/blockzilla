...
[ -x "$indexer" ] || {
    echo "User-program index builder is not executable: $indexer" >&2
    exit 1
}
[ -d "$usage_root" ] && [ -d "$output_root" ] || {
    echo "User-program index input or output root is absent" >&2
    exit 1
}
...
echo "User-program index usage-sorted target batch complete"
