...
# Offline bootstrap helper for the Hetzner -> Blockzilla raw replication trust
...
REPLICA_NODE_ID=${BLOCKZILLA_REPLICA_NODE_ID:-hetzner-dokploy-01}
...
RECEIPT_KEY_ID=${BLOCKZILLA_RECEIPT_KEY_ID:-receipt-2026-q3}
...
mkdir -m 0700 "$stage/offline" "$stage/blockzilla" "$stage/hetzner" "$stage/.work"
work=$stage/.work
...
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:3072 \
  -out "$stage/hetzner/hetzner-replica.key" >/dev/null 2>&1
openssl req -new -sha256 \
  -key "$stage/hetzner/hetzner-replica.key" \
...
openssl x509 -req -sha256 \
  ...
  -out "$stage/hetzner/hetzner-replica.crt" >/dev/null 2>&1
...
openssl pkey \
  ...
  -pubout \
  -out "$stage/hetzner/blockzilla-receipt.pub" >/dev/null 2>&1

cp "$stage/offline/replication-ca.crt" "$stage/blockzilla/replica-ca.crt"
cp "$stage/offline/replication-ca.crt" "$stage/hetzner/blockzilla-primary-ca.crt"
printf '%s\n' "$RECEIPT_KEY_ID" >"$stage/blockzilla/receipt-key-id.txt"
printf '%s\n' "$RECEIPT_KEY_ID" >"$stage/hetzner/receipt-key-id.txt"

client_fingerprint=$(
  openssl x509 -in "$stage/hetzner/hetzner-replica.crt" -outform DER |
...
openssl verify -CAfile "$stage/offline/replication-ca.crt" \
  -purpose sslclient "$stage/hetzner/hetzner-replica.crt" >/dev/null
...
chmod 0600 \
  ...
  "$stage/hetzner/hetzner-replica.key"
chmod 0644 \
  ...
  "$stage/hetzner/blockzilla-primary-ca.crt" \
  "$stage/hetzner/hetzner-replica.crt" \
  "$stage/hetzner/blockzilla-receipt.pub" \
  "$stage/hetzner/receipt-key-id.txt"
