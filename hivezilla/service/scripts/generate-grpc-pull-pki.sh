...
# Add the reverse Blockzilla -> source host pull identities to an existing offline replication CA.
...
ORIGIN_NODE_ID=${BLOCKZILLA_ORIGIN_NODE_ID:-source-node-01}
SOURCE_ID=${BLOCKZILLA_SOURCE_ID:-grpc-raw-source-backup}
SOURCE_SERVER_NAME=${BLOCKZILLA_PULL_SOURCE_SERVER_NAME:-hivezilla-source}
...
RECEIPT_PUBLIC_KEY=$EXISTING_DIRECTORY/source-node/blockzilla-receipt.pub
...
mkdir -m 0700 "$stage/source-node" "$stage/blockzilla" "$stage/.work"
work=$stage/.work

openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:3072 \
  -out "$stage/source-node/source-node-pull-source.key" >/dev/null 2>&1
openssl req -new -sha256 \
  -key "$stage/source-node/source-node-pull-source.key" \
...
openssl x509 -req -sha256 \
  ...
  -out "$stage/source-node/source-node-pull-source.crt" >/dev/null 2>&1
...
printf '%s\n' \
  ...
  '    }' \
  '  ]' \
  '}' >"$stage/source-node/pull-allowed-nodes.json"
cp "$stage/source-node/pull-allowed-nodes.json" "$stage/blockzilla/pull-allowed-nodes.json"
cp "$CA_CERTIFICATE" "$stage/source-node/pull-client-ca.crt"
cp "$CA_CERTIFICATE" "$stage/blockzilla/source-node-pull-source-ca.crt"
cp "$RECEIPT_PUBLIC_KEY" "$stage/source-node/blockzilla-receipt.pub"
cp "$RECEIPT_PUBLIC_KEY" "$stage/blockzilla/blockzilla-receipt.pub"

openssl verify -CAfile "$CA_CERTIFICATE" \
  -purpose sslserver "$stage/source-node/source-node-pull-source.crt" >/dev/null
openssl verify -CAfile "$CA_CERTIFICATE" \
  -purpose sslclient "$stage/blockzilla/blockzilla-pull-client.crt" >/dev/null
openssl pkey -in "$stage/source-node/source-node-pull-source.key" -check -noout >/dev/null
...
rm -rf "$work"
chmod 0600 \
  "$stage/source-node/source-node-pull-source.key" \
  "$stage/blockzilla/blockzilla-pull-client.key"
chmod 0644 \
  "$stage/source-node/source-node-pull-source.crt" \
  "$stage/source-node/pull-client-ca.crt" \
  "$stage/source-node/pull-allowed-nodes.json" \
  "$stage/source-node/blockzilla-receipt.pub" \
  "$stage/blockzilla/blockzilla-pull-client.crt" \
  "$stage/blockzilla/source-node-pull-source-ca.crt" \
