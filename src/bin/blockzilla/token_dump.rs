...
pub async fn dump_token_transactions(
    ...
    let mut first_target_slot_opt: Option<u64> = if processed_epochs.contains(&start_epoch) {
        None
    } else {
        Some(start_slot)
    };
                ...
                for entry_cid_res in block_node.entries.iter() {
                    ...
                    for tx_cid_res in entry.transactions.iter() {
                        ...
                        for key in &account_keys_buf {
                            ...
                            if &key_bytes == &mint_bytes
                                || tracked_account_keys.contains(&key_bytes)
                            {
                                involves_token = true;
                                break;
                            }
